...
> **⚠️ 2026-07-07 감사 노트**: 이 문서는 오래 전에 작성된 이후 갱신되지 않아
> stale한 상태입니다. 실제 코드를 감사한 결과 Sprint 1~7의 개별 Task 대부분
> (worker harassment 응답, 견제 유닛 태그/복귀, 정찰 주기 상수, 25개 빌드
> 패턴 감지, `LURKERMP` 포지셔닝, 뮤탈리스크 마이크로, 거리 캐시,
> `GameConstants`)이 이미 구현되어 있습니다. 실행 전 반드시 먼저 코드베이스에서
> 해당 파일/심볼을 검색해 이미 구현됐는지 확인하세요 — 아래 Task 목록을
> "미구현"으로 가정하고 재작업하지 마십시오. 현재 최신 상태는
> `PLAN-NIGHTLY.md`를 참조하십시오 (테스트 490 pass / 12 skip / 0 fail).
> 실제로 여전히 열려 있는 것으로 확인된 항목: `PLAN-NIGHTLY.md`의 P2.3
> (build_orders.yaml 외부화), P2.5(core 모듈 타입힌트 전체 패스), 그리고
> AI Arena 실전 30연전 승률 검증(Task 8.1 — SC2 게임 클라이언트가 필요해
> 샌드박스에서는 실행 불가).
