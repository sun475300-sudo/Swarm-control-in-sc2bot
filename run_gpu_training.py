...
def run_full_benchmark():
    ...
    combat_result = combat_sim.simulate_battles_gpu(zerglings, marines, num_simulations=4096)
    ...
    start = time.perf_counter()
    combat_result2 = combat_sim.simulate_battles_gpu(roaches, immortals, num_simulations=4096)
    ...
    results["threat_heatmap"] = {"time_ms": heatmap_time, "max": float(threat_map.max())}

    # 영향력 맵
    our_pos = np.random.rand(40, 2) * 200
    our_power = np.random.rand(40) * 15 + 5
    start = time.perf_counter()
    influence = spatial.compute_influence_map(our_pos, our_power, enemy_pos, enemy_dps, resolution=4)
    ...
    # 전략 추론
    game_state = {
        "minerals": 500, "vespene": 200, "supply_used": 80, "supply_cap": 100,
        "worker_count": 40, "army_supply": 40, "enemy_army_supply": 35,
        "base_count": 3, "enemy_base_count": 2, "game_time_minutes": 8,
        "upgrade_count": 3, "tech_level": 2, "threat_level": 1,
        "zergling_count": 20, "roach_count": 8, "hydra_count": 5,
        "ultra_count": 0, "air_count": 0, "score": 15000, "enemy_score": 12000,
    }
    ...
    print(f"  Strategy inference (1000 calls): {strategy_time*1000:.1f}ms total, {strategy_time:.3f}ms avg")
    print(f"    Last: {strategy} (confidence: {conf:.3f}, value: {val:.3f})")
    results["strategy_inference"] = {"total_ms": strategy_time*1000, "avg_us": strategy_time}

    # 배치 전투 추론
    combat_states = [
        {"our_supply": 50, "enemy_supply": 45, "our_hp_ratio": 0.9,
         "enemy_hp_ratio": 0.8, "distance_to_enemy": 8}
        for _ in range(64)
    ]
    start = time.perf_counter()
    ...
    # 배치 마이크로 추론
    unit_states = [
        {"hp_ratio": 0.7, "weapon_cooldown": 0, "distance_to_target": 3,
         "nearby_allies": 8, "nearby_enemies": 5, "on_creep": 1}
        for _ in range(128)
    ]
    start = time.perf_counter()
    ...
    our_units_data = [
        {"x": np.random.rand() * 200, "y": np.random.rand() * 200,
         "hp": 35, "attack": 5, "dps": 10, "supply": 1, "hp_ratio": 0.9,
         "type": "zergling"}
        for _ in range(40)
    ]
    enemy_units_data = [
        {"x": np.random.rand() * 200, "y": np.random.rand() * 200,
         "hp": 45, "attack": 6, "dps": 12, "supply": 1, "hp_ratio": 1.0,
         "type": "marine"}
        for _ in range(20)
    ]
    ...
    start = time.perf_counter()
    for _ in range(100):
        frame_result = integration.process_frame(game_state, our_units_data, enemy_units_data)
    frame_total = (time.perf_counter() - start) * 1000

    print(f"  100 frames processed: {frame_total:.1f}ms total, {frame_total/100:.2f}ms avg/frame")
    ...
    results["frame_processing"] = {
        ...
        "last_frame": {
            k: v for k, v in frame_result.items()
            if k not in ("threat_map", "clusters")
        }
    }
    ...
    print(f"  Combat sim (4096 battles): {results['combat_simulation']['time_ms']:.1f}ms")
    print(f"  Clustering (500 units): {results['clustering']['time_ms']:.1f}ms")
    print(f"  Threat heatmap: {results['threat_heatmap']['time_ms']:.1f}ms")
    print(f"  Strategy inference: {results['strategy_inference']['avg_us']*1000:.3f}ms avg")
    print(f"  Training throughput: {results['training'].get('samples_per_second', 0):,.0f} samples/s")
