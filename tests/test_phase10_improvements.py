...
class TestEconomyGasBanking:
    ...
    def test_gas_overflow_threshold_lowered(self):
        """Gas overflow threshold should be at or below 1000 (originally 3000).

        Lower values mean more aggressive prevention; allow further tuning
        below 1000 (e.g., 800) without breaking the regression contract.
        """
        threshold = self.economy.gas_overflow_prevention_threshold
        assert 0 < threshold <= 1000, (
            f"Expected gas_overflow_prevention_threshold in (0, 1000], got {threshold}"
        )
