"""Stub for sc2.ids.unit_typeid.

Any attribute access returns a deterministic sentinel keyed by the attribute
name, so test code that only compares enum members (e.g. ``unit.type_id ==
UnitTypeId.DRONE``) works without needing every real enum value enumerated.
"""
from __future__ import annotations


class _StubMember:
    __slots__ = ("name",)

    def __init__(self, name: str):
        self.name = name

    def __repr__(self) -> str:
        return f"UnitTypeId.{self.name}"

    def __str__(self) -> str:
        return self.name

    def __eq__(self, other) -> bool:
        if isinstance(other, _StubMember):
            return self.name == other.name
        return False

    def __hash__(self) -> int:
        return hash(("UnitTypeId", self.name))

    def __int__(self) -> int:
        return hash(self.name) & 0xFFFFFFFF


class _UnitTypeIdMeta(type):
    _cache: dict[str, _StubMember] = {}

    def __getattr__(cls, name: str) -> _StubMember:
        if name.startswith("_"):
            raise AttributeError(name)
        member = cls._cache.get(name)
        if member is None:
            member = _StubMember(name)
            cls._cache[name] = member
        return member

    def __iter__(cls):
        return iter(cls._cache.values())


class UnitTypeId(metaclass=_UnitTypeIdMeta):
    """Dynamic stand-in for sc2.ids.unit_typeid.UnitTypeId."""
