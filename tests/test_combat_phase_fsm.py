...
def _run_async(coro):
    ...
    Avoids asyncio.get_event_loop(), which raises RuntimeError on Python
    3.10+ once no event loop is set as current for the thread (e.g. after
    another test in the suite has already closed/consumed one).
    """
