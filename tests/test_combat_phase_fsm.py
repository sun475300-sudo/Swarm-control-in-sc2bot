...
def _run_coro(coro):
    ...
    Using ``asyncio.get_event_loop()`` here is fragile: pytest-asyncio (with
    ``asyncio_mode = auto``) closes the main-thread loop between async tests,
    so a non-async test that calls ``get_event_loop()`` afterwards raises
    ``RuntimeError: There is no current event loop in thread 'MainThread'``.
    We instead spin up a private loop, run the coroutine, and discard it.
    """
