...
class OpenClawMediaCog(commands.Cog, name="OpenClaw 미디어"):
    ...
    async def _reply(
        ...
        title: str,
        text: str,
        color: discord.Color = discord.Color.magenta(),
    ):
    ...
    async def oc_image(
        self, interaction: discord.Interaction, prompt: str, provider: str = "google"
    ):
    ...
    async def oc_image_edit(
        self, interaction: discord.Interaction, prompt: str, image_path: str = ""
    ):
        await interaction.response.defer(thinking=True)
        ...
        await self._safe_run(
            interaction, "✏ 이미지 편집", self.oc.run_skill(msg, timeout=90)
        )

    # ── /oc음성전사 ────────────────────────────────────────────────
    @app_commands.command(
        name="oc음성전사", description="음성 파일을 텍스트로 변환 (Whisper)"
    )
    @app_commands.describe(
        audio_path="오디오 파일 경로 또는 URL", use_api="API 사용 여부"
    )
    async def oc_transcribe(
        self, interaction: discord.Interaction, audio_path: str, use_api: bool = False
    ):
        await interaction.response.defer(thinking=True)
        ...
        await self._safe_run(
            ...
            "🎤 음성 전사 결과",
            self.oc.run_skill(
                f"Transcribe audio file: {audio_path} using {skill}", timeout=120
            ),
            discord.Color.green(),
        )
    ...
    async def oc_gif(self, interaction: discord.Interaction, query: str):
        ...
        await self._safe_run(
            ...
            f"🎞 GIF: {query}",
            self.oc.run_skill(
                f"Search GIFs for: {query}. Download top result to {OUTPUT_DIR}",
                timeout=30,
            ),
    ...
    @app_commands.describe(
        video_path="영상 파일 경로", timestamp="타임스탬프 (예: 00:01:30)"
    )
    async def oc_frames(
        self,
        interaction: discord.Interaction,
