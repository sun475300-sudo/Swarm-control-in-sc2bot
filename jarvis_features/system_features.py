...
class SystemFeaturesCog(commands.Cog, name="시스템 기능"):
    ...
    def _is_path_allowed(self, file_path: str) -> bool:
        ...
        for allowed_dir in self.ALLOWED_FILE_DIRS:
            ...
            try:
                ...
                if (
                    real_path.startswith(norm_allowed + os.sep)
                    or real_path == norm_allowed
                ):
                    return True
            except ValueError:
                continue
    ...
    async def send_file(self, ctx: commands.Context, *, file_path: str):
        ...
        if file_size > 25 * 1024 * 1024:  # Discord 25MB 제한
            await ctx.send(
                f"❌ 파일이 너무 큽니다: {file_size / 1024 / 1024:.1f}MB (최대 25MB)"
            )
            return
        ...
        try:
            ...
            embed.add_field(
                name="파일명", value=os.path.basename(file_path), inline=True
            )
            embed.add_field(
                name="크기", value=f"{file_size / 1024:.1f} KB", inline=True
            )
    ...
    async def receive_file(self, ctx: commands.Context, *, save_path: str = ""):
        ...
        save_dir = (
            save_path.strip('"').strip("'")
            if save_path
            else os.path.join(DATA_DIR, "downloads")
        )
        ...
        if saved_files:
            embed = discord.Embed(
                title="💾 파일 저장 완료", color=discord.Color.green()
            )
    ...
    async def launch_app(self, ctx: commands.Context, *, app_name: str):
        ...
        if executable is None:
            allowed = ", ".join(
                sorted(set(k for k, v in APP_MAP.items() if k == v or not k.isascii()))
            )
    ...
    async def system_dashboard(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            if len(_sys_history) >= 2:
                ...
                cpu_trend = (
                    "📈"
                    if info.get("cpu_percent", 0) > prev.get("cpu_percent", 0)
                    else "📉"
                )
                ram_trend = (
                    "📈"
                    if info.get("ram_percent", 0) > prev.get("ram_percent", 0)
                    else "📉"
                )
                embed.add_field(
                    name="📊 트렌드",
        ...
        async with ctx.typing():
            try:
                ...
                for p in by_ram:
                    ...
                    pid = p.get("pid", 0)
                    ram_lines.append(
                        f"`{pid:>6}` {name:<20} **{mem_pct:.1f}%** ({mem_mb:.0f}MB)"
                    )
                embed.add_field(
                    name="🧠 RAM 사용 상위",
    ...
    async def disk_cleanup(self, ctx: commands.Context, dry_run: str = "yes"):
        ...
        async with ctx.typing():
            ...
            embed.add_field(
                name="__pycache__", value=f"{pycache_count}개 디렉토리", inline=True
            )
            embed.add_field(
                name="총 크기",
            ...
            if not is_dry:
                embed.add_field(
                    name="정리 완료",
                    value=f"{cleaned_size / 1024 / 1024:.1f} MB",
                    inline=True,
                )
            if errors:
                embed.add_field(
                    name="건너뜀", value=f"{errors}개 (사용 중)", inline=True
                )
    ...
    async def network_status(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            ...
            try:
                ...
                embed.add_field(
                    name="📤 전송",
                    value=f"{net.bytes_sent / 1024 / 1024:,.1f} MB",
                    inline=True,
                )
                embed.add_field(
                    name="📥 수신",
                    value=f"{net.bytes_recv / 1024 / 1024:,.1f} MB",
                    inline=True,
                )
                embed.add_field(
                    name="패킷 에러",
                    value=f"TX: {net.errout} / RX: {net.errin}",
                    inline=True,
                )
                ...
                embed.add_field(
                    name="연결 수",
                    value=f"활성: {len(established)} / 리스닝: {len(listening)}",
                    inline=False,
                )
                ...
                for iface, addr_list in addrs.items():
                    for addr in addr_list:
                        if (
                            ...
                            embed.add_field(
                                name=f"🔌 {iface}", value=addr.address, inline=True
                            )
                            break
    ...
    async def smart_home(
        ...
        *,
        ...
        if not device:
            ...
            embed.add_field(
                name="기기 예시", value="조명, 에어컨, TV, 플러그", inline=True
            )
            embed.add_field(
                name="동작 예시", value="켜기, 끄기, 밝기 80, 온도 24", inline=True
            )

            if ha_url:
                embed.add_field(
                    name="Home Assistant", value=f"✅ 연결됨: {ha_url}", inline=False
                )
            else:
                embed.add_field(
                    name="설정 필요",
                    value="HOME_ASSISTANT_URL, HOME_ASSISTANT_TOKEN 환경변수 설정",
                    inline=False,
                )
            await ctx.send(embed=embed)
            return
