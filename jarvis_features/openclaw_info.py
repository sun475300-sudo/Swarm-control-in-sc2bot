...
class OpenClawInfoCog(commands.Cog, name="OpenClaw 정보"):
    ...
    async def oc_weather(
        self, interaction: discord.Interaction, location: str = "서울"
    ):
    ...
    async def oc_exchange(
        ...
        await self._safe_run(
            ...
            f"💱 {from_cur} → {to_cur}",
            self.oc.run_skill(
                f"Get exchange rate from {from_cur} to {to_cur}", timeout=30
            ),
        )
    ...
    async def oc_news(
        self, interaction: discord.Interaction, topic: str = "technology"
    ):
        await interaction.response.defer(thinking=True)
    ...
    async def oc_places(
        self, interaction: discord.Interaction, query: str, location: str = "Seoul"
    ):
        await interaction.response.defer(thinking=True)
        ...
        await self._safe_run(
            ...
            f"📍 장소: {query}",
            self.oc.run_skill(
                f"Search places for '{query}' near {location}", timeout=30
            ),
