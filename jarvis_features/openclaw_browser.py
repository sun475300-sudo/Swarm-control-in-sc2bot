...
class OpenClawBrowserCog(commands.Cog, name="OpenClaw 브라우저"):
    ...
    async def oc_browser_open(self, interaction: discord.Interaction, url: str):
        ...
        await self._safe_run(
            interaction, "🌐 브라우저 열기", self.oc.browser("open", url, timeout=15)
        )

    # ── /oc웹스크린샷 ─────────────────────────────────────────────
    @app_commands.command(name="oc웹스크린샷", description="현재 페이지 스크린샷")
    @app_commands.describe(full_page="전체 페이지 캡처 여부")
    async def oc_browser_screenshot(
        self, interaction: discord.Interaction, full_page: bool = False
    ):
        await interaction.response.defer(thinking=True)
        args = []
        if full_page:
    ...
    @app_commands.command(
        name="oc웹스냅샷", description="페이지 AI 스냅샷 (접근성 트리)"
    )
    ...
    async def oc_browser_snapshot(
        self, interaction: discord.Interaction, format: str = "ai", labels: bool = False
    ):
        await interaction.response.defer(thinking=True)
        args = ["--format", format]
        if labels:
            args.append("--labels")
        await self._safe_run(
            interaction, "🔍 웹 스냅샷", self.oc.browser("snapshot", *args, timeout=15)
        )
    ...
    async def oc_browser_navigate(self, interaction: discord.Interaction, url: str):
        ...
        if not _validate_url(url):
            ...
            return
        await self._safe_run(
            interaction, "➡ 페이지 이동", self.oc.browser("navigate", url, timeout=15)
        )

    # ── /oc웹클릭 ─────────────────────────────────────────────────
    @app_commands.command(name="oc웹클릭", description="웹 요소 클릭 (ref 번호)")
    @app_commands.describe(ref="스냅샷에서 얻은 요소 ref 번호", double="더블클릭 여부")
    async def oc_browser_click(
        self, interaction: discord.Interaction, ref: str, double: bool = False
    ):
        await interaction.response.defer(thinking=True)
    ...
    @app_commands.describe(
        ref="요소 ref 번호", text="입력할 텍스트", submit="Enter 전송 여부"
    )
    async def oc_browser_type(
        self,
        interaction: discord.Interaction,
        ref: str,
        text: str,
        ...
        if submit:
            args.append("--submit")
        await self._safe_run(
            interaction, "⌨ 텍스트 입력", self.oc.browser("type", *args, timeout=10)
        )
    ...
    async def oc_browser_tabs(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True)
        await self._safe_run(
            interaction, "📑 브라우저 탭 목록", self.oc.browser("tabs", timeout=10)
        )

    # ── /oc웹닫기 ─────────────────────────────────────────────────
    @app_commands.command(name="oc웹닫기", description="브라우저 탭 닫기")
    @app_commands.describe(target_id="닫을 탭 ID (비우면 현재 탭)")
    async def oc_browser_close(
        self, interaction: discord.Interaction, target_id: str = ""
    ):
        await interaction.response.defer(thinking=True)
        args = [target_id] if target_id else []
        await self._safe_run(
            interaction, "❌ 탭 닫기", self.oc.browser("close", *args, timeout=10)
        )
    ...
    async def oc_browser_pdf(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True)
        await self._safe_run(
            interaction, "📄 페이지 → PDF", self.oc.browser("pdf", timeout=15)
        )
    ...
    async def oc_browser_status(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True)
        await self._safe_run(
            interaction, "🌐 브라우저 상태", self.oc.browser("status", timeout=10)
        )
