...
class OpenClawDocumentsCog(commands.Cog, name="OpenClaw 문서"):
    ...
    async def _reply_with_file(
        self, interaction: discord.Interaction, title: str, text: str, file_path: str
    ):
        embed = discord.Embed(
            title=title, description=text[:3900], color=discord.Color.teal()
        )
        if os.path.exists(file_path):
            ...
            if file_size > 25 * 1024 * 1024:
                embed.add_field(
                    name="Warning",
                    value=f"File too large ({file_size // 1024 // 1024}MB). Saved to: {file_path}",
                )
                await interaction.followup.send(embed=embed)
            else:
    ...
    async def oc_pdf(self, interaction: discord.Interaction, instruction: str):
        ...
        await self._safe_run(
            ...
            "📄 PDF",
            self.oc.run_skill(
                f"PDF task: {instruction}. Save output to {OUTPUT_DIR}", timeout=60
            ),
        )
    ...
    async def oc_docx(self, interaction: discord.Interaction, instruction: str):
        ...
        await self._safe_run(
            ...
            "📝 Word 문서",
            self.oc.run_skill(
                f"DOCX document task: {instruction}. Save to {OUTPUT_DIR}", timeout=60
            ),
        )
    ...
    async def oc_pptx(
        self, interaction: discord.Interaction, topic: str, slides: int = 5
    ):
        await interaction.response.defer(thinking=True)
