...
class OpenClawDevCog(commands.Cog, name="OpenClaw 개발"):
    ...
    async def _reply(
        ...
        title: str,
        text: str,
        color: discord.Color = discord.Color.dark_blue(),
    ):
    ...
    async def oc_github(
        ...
        await self._safe_run(
            interaction, f"🐙 GitHub ({action})", self.oc.run_skill(msg, timeout=30)
        )

    # ── /oc코딩 ────────────────────────────────────────────────────
    @app_commands.command(
        name="oc코딩", description="코딩 에이전트 (Codex/Claude Code)"
    )
    @app_commands.describe(task="코딩 작업 설명", language="언어")
    async def oc_coding(
        self, interaction: discord.Interaction, task: str, language: str = "python"
    ):
        await interaction.response.defer(thinking=True)
    ...
    async def oc_oracle(
        self, interaction: discord.Interaction, prompt: str, file_path: str = ""
    ):
        await interaction.response.defer(thinking=True)
        ...
        await self._safe_run(
            interaction, "🔮 Oracle 리뷰", self.oc.run_skill(msg, timeout=60)
        )
    ...
    async def oc_calendar(
        self, interaction: discord.Interaction, action: str = "today", details: str = ""
    ):
        await interaction.response.defer(thinking=True)
    ...
    async def oc_stats(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="🦞 OpenClaw 사용 통계", color=discord.Color.blue()
            )
            ...
            top = sorted(
                stats["skill_usage"].items(), key=lambda x: x[1], reverse=True
            )[:5]
            if top:
            ...
            await interaction.followup.send(
                f"통계 조회 오류: {str(e)[:200]}", ephemeral=True
            )
