...
class OpenClawSocialCog(commands.Cog, name="OpenClaw 소셜"):
    ...
    @app_commands.command(
        name="oc트위터", description="Twitter/X 게시 또는 타임라인 조회"
    )
    ...
    async def oc_twitter(
        ...
        await self._safe_run(
            interaction, f"🐦 Twitter ({action})", self.oc.run_skill(msg, timeout=30)
        )
    ...
    async def oc_instagram(
        self, interaction: discord.Interaction, action: str = "feed", content: str = ""
    ):
        await interaction.response.defer(thinking=True)
        ...
        await self._safe_run(
            interaction, f"📸 Instagram ({action})", self.oc.run_skill(msg, timeout=30)
        )

    # ── /oc이메일 ──────────────────────────────────────────────────
    @app_commands.command(name="oc이메일", description="이메일 전송/조회")
    @app_commands.describe(
        action="작업 (send/read/search)", to="수신자", subject="제목", body="본문"
    )
    @app_commands.choices(
        action=[
    ...
    async def oc_email(
        ...
        if action == "send":
            if to and not _EMAIL_RE.match(to):
                await interaction.followup.send(
                    "잘못된 이메일 주소 형식입니다.", ephemeral=True
                )
                return
            # Sanitize fields
            subject = re.sub(r"[\r\n]", "", subject)[:200]
        ...
        await self._safe_run(
            interaction, f"📧 이메일 ({action})", self.oc.run_skill(msg, timeout=30)
        )

    # ── /oc노션 ────────────────────────────────────────────────────
    @app_commands.command(name="oc노션", description="Notion 페이지 생성/조회")
    @app_commands.describe(
        action="작업 (create/search/list)", title="제목", content="내용"
    )
    @app_commands.choices(
        action=[
    ...
    async def oc_notion(
        ...
        await self._safe_run(
            interaction, f"📓 Notion ({action})", self.oc.run_skill(msg, timeout=30)
        )
    ...
    async def oc_slack(
        self, interaction: discord.Interaction, channel: str, message: str
    ):
        await interaction.response.defer(thinking=True)
    ...
    async def oc_trello(
        self, interaction: discord.Interaction, action: str = "list", details: str = ""
    ):
        await interaction.response.defer(thinking=True)
        ...
        await self._safe_run(
            interaction, f"📋 Trello ({action})", self.oc.run_skill(msg, timeout=30)
        )
