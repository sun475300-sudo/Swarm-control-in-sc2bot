...
class ScoringSystem:
    ...
    def on_step(self, iteration: int) -> None:
        ...
        try:
            ...
            logger.debug("scoring evaluation step failed: %s", exc, exc_info=True)
    ...
    def _evaluate_defense(self, game_time: float) -> None:
        ...
        if base_count < self._last_base_count and self._last_base_count > 0:
            ...
            try:
                ...
                logger.debug("near-base enemy count failed: %s", exc)
        ...
        if structures and hasattr(structures, "__call__"):
            try:
                ...
                logger.debug("idle-queen check failed: %s", exc)
        ...
        if game_time < 180:
            ...
            if structures and hasattr(structures, "__call__"):
                try:
                    ...
                    logger.debug("lair/hive tech-up check failed: %s", exc)
        else:
            # 후반: 200 서플 + 공격
            if supply_used >= 180:
    ...
    def _evaluate_micro(self, game_time: float) -> None:
        ...
        try:
            ...
            logger.debug("idle-army micro check failed: %s", exc)
    ...
    def _evaluate_macro(self, game_time: float) -> None:
        ...
        try:
            ...
            logger.debug("creep-tumor spread check failed: %s", exc)
    ...
    def _save_game_score(self, report: Dict) -> None:
        ...
        try:
            ...
            logger.warning("could not persist game scores to %s: %s", filepath, exc)
    ...
    def _load_cumulative_score(self) -> Dict:
        ...
        try:
            ...
            logger.warning("cumulative score file %s unreadable: %s", filepath, exc)
    ...
    def _save_cumulative_score(self) -> None:
        ...
        try:
            ...
            logger.warning("could not save cumulative score to %s: %s", filepath, exc)
