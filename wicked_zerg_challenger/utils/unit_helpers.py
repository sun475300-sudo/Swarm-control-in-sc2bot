...
def _make_units(items):
    """Build a Units collection, falling back to a plain list when sc2 isn't installed."""
    if Units is not None:
        return Units(items, None)
    return list(items)
...
def find_nearby_enemies(unit: Unit, enemy_units: Units, range: float) -> Units:
    ...
    try:
        ...
        if hasattr(enemy_units, "closer_than"):
            ...
            # 폴백: 직접 필터링
            return _make_units([e for e in enemy_units if e.distance_to(unit) < range])
