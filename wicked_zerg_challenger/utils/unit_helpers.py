...
def _empty_units():
    """Return an empty Units-like container.

    Falls back to a plain list when the sc2 dependency is unavailable
    (e.g. in unit tests running without burnysc2 installed).
    """
    if Units is None:
        return []
    return Units([], None)
