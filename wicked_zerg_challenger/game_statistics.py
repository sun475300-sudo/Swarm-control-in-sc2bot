...
class GameStatistics:
    ...
    def record_game(
        self, map_name: str, difficulty: str, enemy_race: str, victory: bool
    ):
        """게임 결과 기록"""
        # 전체 통계
    ...
    def print_statistics(self):
        ...
        for map_name, stats in sorted(self.stats["by_map"].items()):
            ...
            logger.info(
                f"  {map_name:30} | {stats['wins']:3}W {stats['losses']:3}L ({wr:5.1f}%)"
            )
        ...
        for diff, stats in sorted(self.stats["by_difficulty"].items()):
            ...
            logger.info(
                f"  {diff:20} | {stats['wins']:3}W {stats['losses']:3}L ({wr:5.1f}%)"
            )
        ...
        for race, stats in sorted(self.stats["by_race"].items()):
            ...
            logger.info(
                f"  vs {race:15} | {stats['wins']:3}W {stats['losses']:3}L ({wr:5.1f}%)"
            )
        ...
        for key, stats in sorted(self.stats["by_map_difficulty"].items()):
            ...
            logger.info(
                f"  {key:40} | {stats['wins']:3}W {stats['losses']:3}L ({wr:5.1f}%)"
            )
