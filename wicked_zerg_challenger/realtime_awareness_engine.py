...
class RealtimeAwarenessEngine:
    ...
    def on_step(self, iteration: int) -> List[Override]:
        ...
        try:
            ...
            logger.debug("realtime awareness step failed: %s", exc, exc_info=True)
    ...
    def _capture_situation(self, game_time: float) -> None:
        ...
        if townhalls and s.enemy_visible_count > 0:
            try:
                ...
                logger.debug("enemy-near-base scan failed: %s", exc)
        ...
        if UnitTypeId:
            try:
                ...
                logger.debug("tech level lookup failed: %s", exc)
        ...
        if "protoss" in s.enemy_build_pattern.lower() or (
            ...
            if s.army_supply > 0 and s.game_time > 300:
                ...
                try:
                    ...
                    logger.debug("counter-unit count lookup failed: %s", exc)
    ...
    def _force_produce_gas_units(self) -> None:
        ...
        try:
            ...
            logger.debug("emergency mutalisk action failed: %s", exc, exc_info=True)
    ...
    def _force_army_production(self) -> None:
        ...
        try:
            ...
            logger.debug("force-army production failed: %s", exc, exc_info=True)
    ...
    def _force_overlord_production(self) -> None:
        ...
        try:
            ...
            logger.debug("force-overlord production failed: %s", exc, exc_info=True)
    ...
    def _flush_minerals(self) -> None:
        ...
        try:
            ...
            logger.debug("flush-minerals zergling burst failed: %s", exc, exc_info=True)
