...
Google Vertex AI (Gemini)를 활용한 자동 에러 분석 및 패치 제안 시스템

기능:
1. 런타임 에러 발생 시 Traceback 및 소스 코드를 Gemini로 전송
2. Gemini가 원인 분석 및 수정 패치 제안
3. 패치 제안을 로그 파일에 저장 (자동 적용은 선택적)

주의사항:
- 자동 패치 적용은 위험할 수 있으므로 기본적으로 비활성화
...
class GenAISelfHealing:
    """
    Gen-AI Self-Healing 시스템
    
    Google Gemini API를 사용하여 에러를 분석하고 패치를 제안합니다.
    """
    ...
    def __init__(
        ...
        """
        Args:
            api_key: Google Gemini API 키 (환경 변수 GOOGLE_API_KEY에서도 읽음)
            model_name: 사용할 Gemini 모델 이름
            enable_auto_patch: 자동 패치 적용 여부 (기본값: False, 권장하지 않음)
            log_dir: 패치 로그 저장 디렉토리
        """
    ...
    def analyze_error(
        ...
        Args:
            error: 발생한 예외 객체
            context: 추가 컨텍스트 정보 (iteration, game_time, instance_id 등)
            source_files: 관련 소스 파일 내용 (파일 경로 -> 파일 내용)
...
def init_self_healing(
    ...
    Args:
        ...
        enable_auto_patch: 자동 패치 적용 여부 (기본값: False)
