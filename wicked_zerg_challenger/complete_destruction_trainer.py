        ...
        for i in range(num_targets):
            ...
            for unit in units_to_assign:
                try:
                    ...
                    self.logger.debug("multi-target attack assignment for unit %s failed: %s", getattr(unit, 'tag', '?'), exc)
                    continue
        ...
        if nearby_units:
            ...
            for unit in units_to_assign:
                try:
                    ...
                    self.logger.debug("limited destruction attack for unit %s failed: %s", getattr(unit, 'tag', '?'), exc)
                    continue
