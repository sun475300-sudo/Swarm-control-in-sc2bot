...
def show_build_orders():
    ...
    logger.info(f"  [디버그] 파일 경로 확인: {abs_path}")
    if not path.exists():
        logger.info("  [알림] 학습된 빌드 오더 데이터가 아직 없습니다.")
        return
    ...
    try:
        ...
        if timings:
            if "second_base" in timings:
                logger.info(f"  - 앞마당(2멀티): {timings['second_base']:.1f}초 ({int(timings['second_base']//60)}분 {int(timings['second_base']%60)}초)")
            if "third_base" in timings:
                logger.info(f"  - 트리플(3멀티): {timings['third_base']:.1f}초 ({int(timings['third_base']//60)}분 {int(timings['third_base']%60)}초)")
        else:
             logger.info("  (데이터 없음)")

    except Exception as e:
        logger.info(f"  [에러] 데이터 로드 실패: {e}")
...
def show_rl_agent():
    ...
    for path in paths:
        if path.exists():
            try:
                data = np.load(path)
                logger.info(f"  [모델 로드 성공] {path.name}")
                logger.info(f"  [디버그] 모델 키 목록: {list(data.keys())}")
                ...
                if 'q_table' in data:
                    ...
                    if total > 0:
                        for label, count in action_counts.items():
                             logger.info(f"    - {label}: {count}회 ({count/total*100:.1f}%)")
                    else:
                ...
                if 'episode_count' in data:
                     logger.info(f"  - 총 학습 에피소드(게임) 수: {data['episode_count']}")
                
                if 'epsilon' in data:
                     eps = float(data['epsilon'])
                     logger.info(f"  - 현재 탐험률(Epsilon): {eps:.4f} ({eps*100:.1f}%)")
                     if eps > 0.1:
                         logger.info("    → 아직 다양한 전략을 시도해보는 '탐험 단계'입니다.")
                     else:
                         logger.info("    → 학습된 최적 전략을 주로 사용하는 '숙련 단계'입니다.")
                ...
                if 'learning_rate' in data:
                     ...
                     logger.info("  - 현재 학습률(Learning Rate): 0.001 (Adaptive - 상황에 따라 자동 조절됨)")

                # Neural Network Weights
                if 'w1' in data:
                     w1 = data['w1']
                     logger.info(f"  - 신경망 구조: 입력({w1.shape[0]}) -> 은닉({w1.shape[1]}) -> 출력")
                     logger.info(f"  - 가중치 평균/표준편차: {np.mean(w1):.4f} / {np.std(w1):.4f}")
                     logger.info("    (가중치 변화가 있을수록 학습이 진행되고 있음을 의미합니다)")
                
                loaded = True
                break
            except Exception as e:
                logger.info(f"  [에러] {path.name} 읽기 실패: {e}")
