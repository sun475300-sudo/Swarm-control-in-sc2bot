...
프로게이머 리플레이 학습 → 빌드오더 학습 → 게임 훈련에 적용 및 개선

전체 워크플로우:
1. 프로게이머 리플레이에서 빌드오더 추출 및 학습
2. 학습된 빌드오더를 learned_build_orders.json에 저장
3. 게임 훈련 데이터 수집 (선택적)
4. 훈련 데이터와 프로 리플레이 비교 분석
...
class IntegratedReplayLearningWorkflow:
    ...
    def run_full_workflow(self, max_replays: int = 30, skip_training_data: bool = False):
        ...
        print("프로게이머 리플레이 학습 → 빌드오더 학습 → 게임 훈련 적용")
        print("=" * 70)
