...
def test_experience_data_save_load():
    ...
    logger.info(f"Created dummy data: {test_file}")

    # 데이터 로드 확인
    with np.load(str(test_file)) as data:
        logger.info(f"  - States shape: {data['states'].shape}")
        logger.info(f"  - Actions shape: {data['actions'].shape}")
        logger.info(f"  - Rewards shape: {data['rewards'].shape}")
        logger.info("  - Total reward: {np.sum(data['rewards']):.2f}")
...
def test_rl_agent_batch_training():
    ...
    logger.info("  - Loss: {stats['loss']:.4f}")
    logger.info(f"  - Total steps: {stats['steps']}")

    # 모델 저장
    saved = agent.save_model()
    logger.info(f"Model saved: {saved}")

    # 모델 파일 확인
    if temp_model_path.exists():
        logger.info(f"Model file exists: {temp_model_path}")
...
def test_background_learner_processing():
    ...
    # 더미 경험 데이터 생성 (5개 파일)
    for i in range(5):
        file_path = test_buffer_dir / "game_{i:03d}.npz"
        create_dummy_experience_data(file_path, num_steps=25)

    logger.info(f"Created 5 dummy experience files in {test_buffer_dir}")
    ...
    logger.info("BackgroundParallelLearner created with test paths")
    ...
    if processed:
        ...
        logger.info(f"  - Files processed: {learner.stats['files_processed']}")
        logger.info(f"  - Batches trained: {learner.stats['batches_trained']}")
        logger.info(f"  - Total samples: {learner.stats['total_samples']}")
        logger.info("  - Avg loss: {learner.stats['avg_loss']:.4f}")

        # 아카이브 디렉토리 확인
        archived_files = list(test_archive_dir.glob("*.npz"))
        logger.info(f"Archived files: {len(archived_files)}")

        # 버퍼 디렉토리 확인 (처리된 파일은 제거되어야 함)
        remaining_files = list(test_buffer_dir.glob("*.npz"))
        logger.info(f"Remaining files in buffer: {len(remaining_files)}")
    else:
        logger.info("No data processed (may be expected if no files found)")
...
def test_background_learner_lifecycle():
    ...
    logger.info(f"Learner started: {started}")
    logger.info(f"  - Running: {learner.running}")
    logger.info(f"  - Worker thread alive: {learner.worker_thread.is_alive()}")
...
def run_all_tests():
    ...
    try:
        ...
        logger.info("  Total time: {elapsed:.2f}s")
        logger.info("="*70 + "\n")
