...
호출되지만 정의되지 않은 메서드, pass 문만 있는 메서드, TODO 주석을 찾습니다.
"""
...
class MissingLogicChecker:
    ...
    def extract_calls_from_file(self, file_path: Path) -> Set[str]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                # await self._method() 또는 self._method() 형태
                matches = re.findall(
                    r"(?:await\s+)?self\.(_[a-zA-Z_][a-zA-Z0-9_]*)\s*\(", line
                )
                calls.update(matches)

                # await self.method() 또는 self.method() 형태 (public methods)
                matches2 = re.findall(
    ...
    def find_pass_statements(self, file_path: Path) -> List[int]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                stripped = line.strip()
                # 단독 pass 문 찾기 (주석이나 다른 코드와 함께 있는 경우 제외)
                if stripped == "pass" or (
...
def main():
    ...
    logger.info(f"  - pass 문만 있는 파일: {results['files_with_pass']}개")
    ...
    if results["pass_statements"]:
        logger.info("\n" + "=" * 70)
        logger.info("pass 문만 있는 파일 목록 (상위 10개):")
        logger.info("=" * 70)
