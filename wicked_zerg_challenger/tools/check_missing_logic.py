...
호출되었지만 정의되지 않은 메서드, pass만 있는 메서드, TODO 주석을 찾습니다.
"""
...
class MissingLogicChecker:
    ...
    def find_pass_statements(self, file_path: Path) -> List[int]:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                stripped = line.strip()
                # 단독 pass 문만 찾기 (주석이나 다른 코드와 함께 있는 것은 제외)
                if stripped == "pass" or (
...
def main():
    ...
    if results["pass_statements"]:
        logger.info("\n" + "=" * 70)
        logger.info("pass만 있는 파일 목록 (상위 10개):")
        logger.info("=" * 70)
        ...
        for file_path, lines in sorted_files:
            logger.info(f"\n{file_path}: {len(lines)}개 pass 줄")
            if len(lines) <= 20:
                logger.info(f"  줄번호: {', '.join(map(str, lines[:20]))}")
            else:
                logger.info(
                    f"  줄번호: {', '.join(map(str, lines[:20]))} ... (총 {len(lines)}개)"
                )
