# -*- coding: utf-8 -*-
"""
Missing implementation checker.

Finds methods that are called but never defined, methods whose body is only
`pass`, and TODO comments.
"""
...
class MissingLogicChecker:
    """Scans the project for missing implementations."""
    ...
    def extract_methods_from_file(self, file_path: Path) -> Set[str]:
        """Extract method names defined in a file."""
        methods = set()
        try:
    ...
    def extract_calls_from_file(self, file_path: Path) -> Set[str]:
        """Extract method names called via self.* in a file."""
        calls = set()
        try:
            with open(file_path, "r", encoding="utf-8", errors="replace") as f:
                content = f.read()
            ...
            # Find self._method() calls
            for i, line in enumerate(lines, 1):
                # await self._method() or self._method()
                matches = re.findall(
    ...
    def find_pass_statements(self, file_path: Path) -> List[int]:
        """Find lines whose body is just `pass`."""
        pass_lines = []
        try:
            with open(file_path, "r", encoding="utf-8", errors="replace") as f:
                lines = f.readlines()
            ...
            for i, line in enumerate(lines, 1):
                stripped = line.strip()
                # Only bare `pass` lines (excludes ones combined with a comment/other code)
                if stripped == "pass" or (
                    stripped.startswith("pass") and len(stripped) == 4
                ):
                    # Confirm it's directly inside a function/method body
    ...
    def scan_file(self, file_path: Path):
        ...
        # Find methods called in this file but never defined in it
        ...
        for file_path, called in self.called_methods.items():
            for method in called:
                if method not in all_defined and method.startswith("_"):
                    # private method is called but never defined anywhere
                    self.missing_implementations.append(
                        {
                            "file": file_path,
                            "method": method,
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("Missing implementation checker")
    logger.info("=" * 70)
    ...
    logger.info(f"  - Files with `pass` bodies: {results['files_with_pass']}")
    ...
    # Files with the most `pass`-only bodies
    if results["pass_statements"]:
        logger.info("\n" + "=" * 70)
        logger.info("Files with the most `pass` bodies (top 10):")
        logger.info("=" * 70)
        ...
        for file_path, lines in sorted_files:
            logger.info(f"\n{file_path}: {len(lines)} pass statements")
            if len(lines) <= 20:
