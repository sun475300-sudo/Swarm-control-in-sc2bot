...
class ReplayPipeline:
    def __init__(self):
        ...
        self.archive_dir = (
            self.base_dir / "data" / "replays" / "archive"
        )  # Legacy support
    ...
    def scrape_replay_links(self, limit: int = 5) -> List[str]:
        ...
        url = (
            "https://lotv.spawningtool.com/replays/?race=1&tag=9"  # Zerg (1) + Pro (9)
        )
        ...
        try:
            ...
            links_found = re.findall(
                r'href=["\'](/replays/download/\d+/|/replays/\d+/|/\d+/)["\']',
                response.text,
            )
    ...
    def download_file(self, url: str) -> Optional[Path]:
        ...
        try:
            ...
            response = requests.get(
                url, headers=self.headers, verify=True, stream=True, timeout=60
            )
            ...
            if (
                final_url.lower().endswith(".sc2replay")
                or ".sc2replay?" in final_url.lower()
            ):
    ...
    def run_pipeline(self, num_replays=5, epochs=5):
        ...
        learner = ReplayBuildOrderLearner(
            replay_dir=str(self.replays_dir), output_dir=None  # Use default
        )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of replays to download"
    )
    parser.add_argument(
