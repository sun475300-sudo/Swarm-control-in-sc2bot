...
리플레이 학습 데이터와 훈련 리플레이 학습 데이터를 비교 분석하고,
...
def apply_learned_differences(
    ...
    for param_name, comp in timing_comparisons.items():
        ...
        # If training is significantly later than pro (diff > 5), adjust towards pro
        if diff > 5:
            ...
            print(f"[LEARNING] {param_name}: {current_value:.1f} → {updated_params[param_name]:.1f} (target: {pro_mean:.1f}, diff: {diff:.1f})")
...
def main():
    ...
    if improvements_applied:
        ...
        try:
            extractor = ReplayBuildOrderExtractor(replay_dir=str(comparator.pro_replay_dir))
            ...
            if learned_params:
                ...
                print(f"[SUCCESS] Learned {len(learned_params)} parameters from {max_replays} replays")
            else:
