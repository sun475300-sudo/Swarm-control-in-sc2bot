...
class BuildOrderComparison:
    ...
    # training - pro (positive = later, negative = earlier)
...
class BuildOrderComparator:
 ...
 - Update learned parameters for next game
    ...
    def __init__(self, learned_data_path: Optional[Path] = None):
        ...
        self.comparison_history_file = Path(
    ...
    def _load_pro_baseline(self) -> None:
        ...
        if self.learned_data_path is None or not self.learned_data_path.exists():
            print(
                f"[WARNING] Learned data file not found: {self.learned_data_path}")
            return
        ...
        try:
            ...
            print(
                f"[BUILD COMPARATOR] Loaded pro baseline: {len(self.pro_baseline)} parameters")
    ...
    def compare(
        ...
        for param_name in [
            ...
            comparison = self._compare_parameter(
                param_name, training_value, pro_value, game_result)
    ...
    def _compare_parameter(
        ...
        if training_value is None:
            return BuildOrderComparison(
                ...
                recommendation=f"WARNING {param_name}: Not executed. Should execute at supply {pro_value}"
            )
        ...
        if pro_value is None:
            return BuildOrderComparison(
                ...
                recommendation=f"INFO {param_name}: Executed at supply {training_value} (no baseline)"
            )
        ...
        if abs(difference) <= tolerance:
            recommendation = f"OK {param_name}: Excellent timing (Training: {training_value}, Pro: {pro_value})"
        elif difference > tolerance:
            # Training is later than pro (bad if defeat, okay if victory)
            if game_result == "Defeat":
                improvement_needed = True
                recommendation = f"WARNING {param_name}: Too late (Training: {training_value}, Pro: {pro_value}, Gap: +{difference:.1f})"
            else:
                recommendation = f"INFO {param_name}: Later than pro but won (Training: {training_value}, Pro: {pro_value})"
        else:
            # Training is earlier than pro (generally good)
            if game_result == "Victory":
                recommendation = f"OK {param_name}: Earlier than pro and won (Training: {training_value}, Pro: {pro_value}, Gap: {difference:.1f})"
            else:
                recommendation = f"WARNING {param_name}: Earlier than pro but lost (Training: {training_value}, Pro: {pro_value})"
