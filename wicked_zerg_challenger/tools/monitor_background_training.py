...
class BackgroundTrainingMonitor:
    ...
    def print_dashboard(self):
        ...
        logger.info("=" * 80)
        logger.info(
            f"Monitoring Started: {datetime.fromtimestamp(self.start_time).strftime('%Y-%m-%d %H:%M:%S')}"
        )
        logger.info(
            f"Current Time:       {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        logger.info(
            f"Uptime:             {self.format_time_delta(time.time() - self.start_time)}"
        )
        logger.info("=" * 80)
        ...
        if buffer_delta > 0:
            ...
            logger.info("Status:      [O] No change")
        ...
        if archive_delta > 0:
            logger.info(
                f"Status:      [OK] +{archive_delta} files archived (training completed)"
            )
        else:
            logger.info("Status:      [O] No change")
        ...
        if model_info["exists"]:
            ...
            logger.info(
                f"Modified:    {model_info['modified'].strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            if self.last_model_mtime > 0 and current_mtime > self.last_model_mtime:
                ...
                logger.info(
                    f"Update:      ? Model updated {self.format_time_delta(time_since_update)} ago!"
                )
    ...
    def run(self):
        ...
        try:
            ...
            logger.info(
                f"Total monitoring time: {self.format_time_delta(time.time() - self.start_time)}"
            )
