...
class BuildFeedbackSystem:
    ...
    def _analyze_and_improve(self):
        ...
        try:
            ...
            logger.info(
                "  Win Rate: {win_rate * 100:.1f}% ({len(victories)}/{len(recent_games)})"
            )
            logger.info("  Avg Victory Time: {avg_victory_time:.0f}s")
            ...
            for build, stats in build_stats.items():
                ...
                logger.info(
                    "    {build}: {wr * 100:.1f}% ({stats['wins']}/{stats['total']})"
                )
    ...
    def _generate_recommendations(
        ...
        if avg_victory_time > 600:  # 10분 이상
            logger.info(
                "  - 승리가 너무 느림 ({avg_victory_time:.0f}s) → 더 공격적인 전략 필요"
            )
            logger.info("    → 제안: 3분 저글링 공격, 5분 바퀴 푸시")
        elif avg_victory_time < 300:  # 5분 미만
            logger.info(
                "  - 매우 빠른 승리! ({avg_victory_time:.0f}s) → 현재 전략 유지"
            )
                ...
                if resources:
                    ...
                    if mid_game:
                        ...
                        if avg_minerals > 1000:
                            logger.info(
                                "  - 미네랄 과잉 ({avg_minerals:.0f}) → 유닛 생산 증가 필요"
                            )
