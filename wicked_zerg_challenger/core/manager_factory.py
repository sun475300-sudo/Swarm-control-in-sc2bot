...
class ManagerFactory:
    ...
    def register_manager(self, config: ManagerConfig) -> None:
        ...
        """
        if config is None:
            logger.warning("register_manager: config is None, skipping")
            return
    ...
    def register_managers(self, configs: List[ManagerConfig]) -> None:
        ...
        """
        if configs is None:
            logger.warning("register_managers: configs is None, skipping")
            return
    ...
    def _initialize_manager(self, config: ManagerConfig, verbose: bool) -> bool:
        ...
        try:
            ...
            if verbose:
                logger.warning("[BOT_WARN] %s not available: %s", config.name, e)
