...
class SituationalAwareness:
    ...
    def update_sitrep(self):
        ...
        if (
            ...
            and StrategyManagerV2
            and isinstance(self.bot.strategy_manager, StrategyManagerV2)
        ):
        ...
        sitrep = {
            ...
            "economy": {
                ...
                "workers": (
                    getattr(self.bot, "workers", []).amount
                    if hasattr(self.bot, "workers")
                    else 0
                ),
                "bases": (
                    getattr(self.bot, "townhalls", []).amount
    ...
    def _assess_opportunity(self) -> OpportunityIndex:
        ...
        # Army advantage?
        if hasattr(self.bot, "strategy_manager") and isinstance(
            self.bot.strategy_manager, StrategyManagerV2
        ):
            if self.bot.strategy_manager.current_win_condition.name in [
                "WINNING_ARMY",
                "WINNING_ECONOMY",
            ]:
    ...
    def _get_game_phase(self) -> str:
        ...
        if hasattr(self.bot, "strategy_manager") and hasattr(
            self.bot.strategy_manager, "current_build_phase"
        ):
    ...
    def _get_intel_summary(self) -> Dict[str, Any]:
        ...
        # 1. Cloak Detection
        cloak_tech = {"DARKSHRINE", "GHOSTACADEMY", "LURKERDEN", "LURKERDENMP"}
        ...
        # 3. Splash Damage
        splash_tech = {"BANELINGNEST", "ROBOTICSBAY", "TEMPLARARCHIVE", "FUSIONCORE"}
        ...
        return {
            ...
            "last_seen_threat_frame": (
                getattr(self.bot, "iteration", 0) if detected_threats else 0
            ),
        }
