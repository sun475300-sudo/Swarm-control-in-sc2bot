...
class UnitFactory:
    ...
    def _update_gas_ratio_target(self) -> None:
        ...
        ??? ???ろ꼥??????ㅻ깹????좊읈??????ル늅筌??????????깆뱾 ?釉뚰???
        """
        # Strategy Manager: use detected enemy race for gas ratio target
        strategy = getattr(self.bot, "strategy_manager", None)
        if strategy:
    ...
    def _check_combat_mode(self, iteration: int) -> bool:
        ...
        # Combat mode flag (reset each evaluation)
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        # Hatchery pending check (counts in-progress hatcheries)
        ...
        if self.blackboard:
            ...
            # Aggregate requested units before sending to blackboard
            unit_requests = {}
