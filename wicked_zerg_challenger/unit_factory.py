...
class UnitFactory:
    ...
    def _update_gas_ratio_target(self) -> None:
        ...
        ??? ???ろ꼥??????ㅻ깹????좊읈??????ル늅筌??????????깆뱾 ?釉뚰???
        """
        # Strategy Manager from bot if available
        strategy = getattr(self.bot, "strategy_manager", None)
        if strategy:
    ...
    def _check_combat_mode(self, iteration: int) -> bool:
        ...
        # (corrupted Korean text removed; assignment above is the real one)
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        if self.blackboard:
            ...
            # initialize unit_requests for blackboard production batching
            unit_requests = {}
