...
class UnitFactory:
    def __init__(self, bot, blackboard=None, config=None):
        ...
        # Track combat-mode reinforcement decisions.
    ...
    def _update_gas_ratio_target(self) -> None:
        ...
        ??? ???ろ꼥??????ㅻ깹????좊읈??????ル늅筌??????????깆뱾 ?釉뚰???
        """
        # Detect race via Strategy Manager and update gas ratio target.
        strategy = getattr(self.bot, "strategy_manager", None)
        if strategy:
    ...
    def _check_combat_mode(self, iteration: int) -> bool:
        ...
        # Re-evaluate combat indicators below.
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        # pending_hatch already computed above.
        ...
        if self.blackboard:
            ...
            # Aggregate larva production requests by unit type.
            unit_requests = {}
