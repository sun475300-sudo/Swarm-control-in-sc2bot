...
class UnitFactory:
    def __init__(self, bot, blackboard=None, config=None):
        ...
        # combat mode flag is initialized on the next line
    ...
    def _update_gas_ratio_target(self) -> None:
        ...
        ??? ???ろ꼥??????ㅻ깹????좊읈??????ル늅筌??????????깆뱾 ?釉뚰???
        """
        # Strategy Manager: read detected enemy race
        strategy = getattr(self.bot, "strategy_manager", None)
        if strategy:
    ...
    def _check_combat_mode(self, iteration: int) -> bool:
        ...
        # in_combat: see strategy manager + nearby enemies below
    ...
    async def on_step(self, iteration: int) -> None:
        ...
        # pending_hatch: counts already-queued hatcheries
        ...
        if self.blackboard:
            ...
            # collect requested unit types into a multiset
            unit_requests = {}
