...
def has_lair(bot) -> bool:
    ...
    return (
        bot.structures(UnitTypeId.LAIR).exists or bot.structures(UnitTypeId.HIVE).exists
    )
...
def create_zerg_behavior_tree() -> BehaviorTree:
    ...
    early_strategy = create_sequence(
        "초반 전략", create_condition("초반?", is_early_game), early_actions
    )
    ...
    mid_strategy = create_sequence(
        "중반 전략", create_condition("중반?", is_mid_game), mid_actions
    )
    ...
    late_actions = create_selector(
        "후반 액션", victory_push_sequence, create_action("표준 공격", standard_attack)
    )

    late_strategy = create_sequence(
        "후반 전략", create_condition("후반?", is_late_game), late_actions
    )
