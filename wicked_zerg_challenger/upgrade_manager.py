...
def _upgrade_id(name: str):
    value = getattr(UpgradeId, name, None) if UpgradeId else None
...
MATCHUP_UPGRADE_PRIORITY = {
    "ZvT": {
        "early": [
            _upgrade_id("ZERGLINGMOVEMENTSPEED"),
        ],
        "mid": [
            _upgrade_id("ZERGMELEEWEAPONSLEVEL1"),
            _upgrade_id("ZERGGROUNDARMORSLEVEL1"),
            _upgrade_id("CENTRIFICALHOOKS"),
        ],
        "late": [
            _upgrade_id("ZERGMELEEWEAPONSLEVEL3"),
            _upgrade_id("CHITINOUSPLATING"),
            _upgrade_id("ANABOLICSYNTHESIS"),
        ],
    },
    "ZvP": {
        "early": [
        ...
        "mid": [
            _upgrade_id("ZERGMISSILEWEAPONSLEVEL1"),
            _upgrade_id("EVOLVEGROOVEDSPINES"),
            _upgrade_id("ZERGGROUNDARMORSLEVEL1"),
        ],
        "late": [
            _upgrade_id("ZERGMISSILEWEAPONSLEVEL3"),
            _upgrade_id("ZERGFLYERWEAPONSLEVEL1"),
            _upgrade_id("LURKERRANGE"),
        ],
    },
    "ZvZ": {
        "early": [
        ...
        "mid": [
            _upgrade_id("ZERGMISSILEWEAPONSLEVEL1"),
            _upgrade_id("GLIALRECONSTITUTION"),
            _upgrade_id("ZERGGROUNDARMORSLEVEL1"),
        ],
        "late": [
            _upgrade_id("ZERGFLYERWEAPONSLEVEL1"),
            _upgrade_id("ZERGFLYERARMORSLEVEL1"),
            _upgrade_id("INFESTORENERGYUPGRADE"),
        ],
    },
}
...
class EvolutionUpgradeManager:
    ...
    def _get_upgrade_priority(self) -> List[object]:
        ...
        * Phase 18: Get upgrade priority with Intel integration *
        ...
        matchup_order = self.get_matchup_upgrade_priority()
        if not matchup_order:
            return upgrade_order

        ordered: List[object] = []
        seen = set()
        for upgrade in matchup_order + upgrade_order:
            if not upgrade or upgrade in seen:
                continue
            ordered.append(upgrade)
            seen.add(upgrade)
        return ordered

    def get_matchup_upgrade_priority(
        self, matchup: Optional[str] = None, phase: Optional[str] = None
    ) -> List[object]:
        """Return STRATEGY_PLAN Phase 4 matchup-specific upgrade priority."""
        matchup = matchup or self._get_matchup_key()
        phase = phase or self._get_upgrade_phase()
        return list(MATCHUP_UPGRADE_PRIORITY.get(matchup, {}).get(phase, []))

    def _get_matchup_key(self) -> str:
        enemy_race = self._normalize_enemy_race(getattr(self.bot, "enemy_race", ""))
        if "terran" in enemy_race:
            return "ZvT"
        if "protoss" in enemy_race:
            return "ZvP"
        if "zerg" in enemy_race:
            return "ZvZ"
        return "ZvT"

    def _get_upgrade_phase(self) -> str:
    ...
    async def _upgrade_to_lair(self, iteration: int) -> None:
        ...
        if self.bot.minerals < 200 or self.bot.vespene < 100:
            if iteration % 100 == 0:
                self.logger.info(
                    f"[{int(self.bot.time)}s] [WAIT] Lair 자원 대기: {self.bot.minerals}m/{self.bot.vespene}g (필요: 150m/100g)"
                )
            return
