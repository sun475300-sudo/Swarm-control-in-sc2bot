...
class TestProductionResilience(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_counter_unit_terran_marine(self):
        ...
        # Sync method: takes the enemy unit list plus tech-gate flags directly
        result = self.resilience._get_counter_unit(
            [mock_marine], has_roach_warren=True, has_hydra_den=True, has_spire=True
        )

        # Light infantry counter falls through to Roach (baneling morph path
        # is intentionally not handled inside this helper)
    ...
    async def test_get_counter_unit_protoss(self):
        ...
        # Armored ground prefers Hydralisk over Roach when the den is ready
    ...
    async def test_get_counter_unit_zerg(self):
        ...
        result = self.resilience._get_counter_unit(
            [mock_muta], has_roach_warren=True, has_hydra_den=True, has_spire=True
        )

        # Air threat prefers Hydralisk over Mutalisk when the den is ready
        self.assertEqual(result, UnitTypeId.HYDRALISK)
