...
class TestProductionResilience(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_counter_unit_terran_marine(self):
        ...
        mock_marine = Mock()
        mock_marine.type_id = SimpleNamespace(name="MARINE")
        enemy_units = [mock_marine] * 5

        result = self.resilience._get_counter_unit(
            enemy_units, has_roach_warren=True, has_hydra_den=False, has_spire=False
        )

        # vs light infantry: BANELING (no nest) or ROACH or ZERGLING
        valid_counters = [
            UnitTypeId.BANELING,
            UnitTypeId.ROACH,
            UnitTypeId.ZERGLING,
    ...
    def test_get_counter_unit_protoss(self):
        ...
        mock_stalker = Mock()
        mock_stalker.type_id = SimpleNamespace(name="STALKER")
    ...
    def test_get_counter_unit_zerg(self):
        ...
        mock_muta = Mock()
        mock_muta.type_id = SimpleNamespace(name="MUTALISK")
        enemy_units = [mock_muta] * 6
