...
class TestProductionResilience(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_counter_unit_terran_marine(self):
        """Test counter selection against a Terran marine-heavy composition"""
        mock_marine = Mock()
        mock_marine.type_id = UnitTypeId.MARINE

        result = self.resilience._get_counter_unit(
            [mock_marine], has_roach_warren=True, has_hydra_den=False, has_spire=False
        )

        # Light infantry with only a Roach Warren available -> Roach
        self.assertEqual(result, UnitTypeId.ROACH)

    def test_get_counter_unit_protoss(self):
        """Test counter selection against a Protoss stalker-heavy composition"""
        mock_stalker = Mock()
        mock_stalker.type_id = UnitTypeId.STALKER

        result = self.resilience._get_counter_unit(
            [mock_stalker], has_roach_warren=False, has_hydra_den=True, has_spire=False
        )

        # Armored ground with only a Hydralisk Den available -> Hydralisk
        self.assertEqual(result, UnitTypeId.HYDRALISK)

    def test_get_counter_unit_zerg(self):
        """Test counter selection against a Zerg mutalisk-heavy composition"""
        mock_muta = Mock()
        mock_muta.type_id = UnitTypeId.MUTALISK

        result = self.resilience._get_counter_unit(
            [mock_muta], has_roach_warren=False, has_hydra_den=True, has_spire=False
        )

        # Air threat with only a Hydralisk Den available -> Hydralisk
        self.assertEqual(result, UnitTypeId.HYDRALISK)
