...
class TestProductionResilience(unittest.IsolatedAsyncioTestCase):
    ...
    Uses IsolatedAsyncioTestCase so the 9 `async def test_…` methods
    in this class actually execute. Plain unittest.TestCase silently
    treats them as coroutine-returning functions and skips the body.
    """
    ...
    def _make_enemy_unit(self, type_id):
        mock = Mock()
        mock.type_id = Mock()
        mock.type_id.name = type_id.name
        return mock

    def test_get_counter_unit_terran_marine(self):
        """Test counter selection against Terran marines (light infantry)"""
        enemy_units = [self._make_enemy_unit(UnitTypeId.MARINE) for _ in range(4)]

        # With Roach Warren but no Baneling Nest -> Roach
    ...
    def test_get_counter_unit_protoss(self):
        ...
        enemy_units = [self._make_enemy_unit(UnitTypeId.STALKER) for _ in range(4)]
