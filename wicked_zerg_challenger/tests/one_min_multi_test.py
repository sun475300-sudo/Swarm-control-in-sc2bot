...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("1-Minute Multi Timing Test")
    logger.info("=" * 60)
    logger.info("\nThis test validates that the bot:")
    logger.info("  1. Places natural expansion Hatchery by 1:00 game time")
    logger.info("  2. Has sufficient minerals (300+) at placement")
    logger.info("  3. Only expands when safe (no early aggression)")
    logger.info("\nTo use this test:")
    logger.info("  1. Import into your bot: from tests.one_min_multi_test import OneMinMultiTest")
    logger.info("  2. Initialize in bot __init__: self.multi_test = OneMinMultiTest(self)")
    logger.info("  3. Call in on_step: await self.multi_test.on_step(iteration)")
    logger.info("  4. Check results after game: self.multi_test.print_results()")
