...
def remove_lock():
    ...
    try:
        ...
        logger.debug("remove_lock failed: %s", e)
...
def run_single_game(
    ...
    try:
        ...
        logger.info(
            f"Map: {map_name} | Difficulty: {difficulty_name} | Enemy: {enemy_race.name}"
        )
        logger.info(f"{'='*70}\n")

        result = run_game(
            maps.get(map_name),
            [Bot(Race.Zerg, WickedZergBotProImpl()), Computer(enemy_race, difficulty)],
            realtime=False,
            save_replay_as=None,
...
def calculate_win_rate(stats: GameStatistics, difficulty_name: str, race_name: str):
    ...
    Note: GameStatistics currently tracks wins/losses by difficulty and by
    race independently (no combined (difficulty, race) bucket exists), so
    this returns the difficulty-level win rate.  The race_name parameter is
    preserved to keep the calling signature stable and to allow a future
    combined breakdown without another touch.
    """
    # 난이도별 통계 확인
...
def train_difficulty_with_random_races(
    difficulty: Difficulty, difficulty_name: str, stats: GameStatistics
):
    """특정 난이도에서 랜덤 종족으로 학습 (각 종족 90% 유지)"""
    logger.info("\n" + "=" * 80)
    logger.info(f"TRAINING: {difficulty_name} with RANDOM races")
    logger.info(
        f"Target: {TARGET_WIN_RATE}% win rate per race in {GAMES_PER_COMBINATION} games"
    )
    logger.info("=" * 80)
    ...
    for enemy_race in RACES:
        ...
        while attempt <= max_attempts:
            ...
            for game_num in range(1, GAMES_PER_COMBINATION + 1):
                ...
                logger.info(
                    f"    Game {game_num}: {wins}/{game_num} ({current_wr:.1f}%)"
                )
            ...
            logger.info(
                f"  [RESULT] vs {enemy_race.name}: {wins}/{GAMES_PER_COMBINATION} ({final_wr:.1f}%)"
            )
            ...
            if final_wr >= TARGET_WIN_RATE:
                ...
                logger.info(
                    f"  [RETRY] Below target ({final_wr:.1f}% < {TARGET_WIN_RATE}%)"
                )
                attempt += 1
        ...
        if enemy_race.name not in race_results:
            race_results[enemy_race.name] = final_wr
            logger.warning(
                f"  [WARNING] Could not achieve target for {enemy_race.name} after {max_attempts} attempts"
            )
...
def main():
    ...
    for difficulty, difficulty_name in DIFFICULTIES:
        ...
        success = train_difficulty_with_random_races(difficulty, difficulty_name, stats)
