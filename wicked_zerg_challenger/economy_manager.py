...
class EconomyManager:
    ...
    async def _optimize_early_worker_split(self) -> None:
        ...
        sorted_minerals = sorted(
            nearby_minerals, key=lambda m: m.distance_to(main_base)
        )
    ...
    async def _train_drone_if_needed(self) -> None:
        ...
        if (
            ...
            and pending_hatch == 0
            and worker_count >= 13
            and current_minerals >= 200
            and game_time_check > 60
        ):
    ...
    # NOTE: an earlier draft of `_prevent_resource_banking` lived here.
    # The canonical, blackboard-aware implementation later in this file
    # (game-time / threat-level gated, with macro-hatchery support and
    # broader mineral/gas thresholds) shadowed this older stub via
    ...
    # NOTE: an earlier draft of `_reduce_gas_workers` lived here. It only
    # moved a single worker per call and used the basic 3-harvester check.
    # The canonical implementation lower in this file (severity-based
    # min-worker thresholds: 0/1/2 depending on gas-bank size) overwrote
    # it via Python's class-body redefinition rule, so this stub was dead
    # code. Removed.
