...
class EconomyManager:
    ...
    # NOTE: 이 클래스의 ``_prevent_resource_banking`` 활성 구현은 후반부
    # (Macro Hatchery + 스마트 확장 분기)에 있다. 이전에는 여기에도 같은 이름의
    # 구현(extra Queen + static defense 분기)이 있었으나 동일 클래스 내에서
    # 후방의 재정의에 의해 통째로 오버라이드되어 죽은 코드였다. 제거.
    ...
    # NOTE: 이 클래스의 ``_reduce_gas_workers`` 활성 구현은 후반부
    # (가스 뱅킹 심각도 기반 min_workers 단계)에 있다. 이전에는 여기에도
    # 단순한 1-명-이동 구현이 있었으나 후방의 재정의에 의해 통째로
    # 오버라이드되어 죽은 코드였다. 제거.
