...
class EconomyManager:
    ...
    def __init__(self, bot):
        ...
        # ★ Gas-banking trigger for macro hatchery ★
        # When vespene >= this AND larva is starved, treat as gas-banking
        # and lower the macro-hatchery mineral bar (see _build_macro_hatchery).
        # Tightened 1500 -> 1200 so we react before 700 extra gas accumulates
        # past the worker-redirect trigger (`gas_overflow_prevention_threshold`).
        self.gas_banking_macro_hatch_trigger = 1200
        self.gas_banking_larva_floor = 5  # avg larva below this with banked gas
    ...
    async def _build_macro_hatchery_if_needed(self) -> None:
        ...
        # 가스 과잉 체크 (가스 banking + 라바 부족 → 매크로 해처리 가속)
        gas = getattr(self.bot, "vespene", 0)
        total_larva = len(self.bot.larva) if hasattr(self.bot, "larva") else 0
        if (
            gas >= self.gas_banking_macro_hatch_trigger
            and total_larva < self.gas_banking_larva_floor
        ):
            gas_overflow = True
