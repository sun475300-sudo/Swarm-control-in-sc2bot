...
class EconomyManager:
    ...
    # NOTE: An older `_prevent_resource_banking` implementation lived here
    # which only handled Queen/Spine/Spore production. It was shadowed by the
    # blackboard-aware version defined later in this class and so was never
    # invoked. It has been removed to avoid confusion.
    ...
    # NOTE: An older `_reduce_gas_workers` lived here that always pulled at
    # most one worker. The later, severity-aware version (which scales the
    # minimum-kept-on-gas count by current gas reserves) shadowed it, so this
    # implementation was dead code. Removed.
