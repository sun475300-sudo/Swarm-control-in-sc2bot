...
class EconomyManager:
    ...
    async def _prevent_resource_banking_via_defense(self) -> None:
        """
        * Prevent resource banking by spending excess minerals — DEFENSE PATH *

        NOTE (sweep #14): originally named `_prevent_resource_banking` but
        was being silently shadowed by the macro-hatchery/expansion version
        defined later in this class (line ~3286). Renamed so both paths are
        callable; nothing currently calls this one, but the queens + static
        defense logic is intentionally preserved here in case a future
        strategy wants it. Decide on consolidation when you have a clear
        signal.
    ...
    async def _reduce_gas_workers_simple(self) -> None:
        """가스 일꾼 감소 (과잉 가스 방지)

        NOTE (sweep #14): originally named `_reduce_gas_workers` but was
        being silently shadowed by the severity-aware version at line
        ~4118. Renamed so the simpler "drop one worker per saturated
        extractor" behaviour is still available if a caller wants it.
        """
        try:
            if (
