...
class EconomyManager:
    ...
    async def _build_static_defense_when_banking(self) -> None:
        """
        Spend excess minerals on static defense once safe to do so.

        Renamed from a duplicate ``_prevent_resource_banking`` that was
        silently shadowed (F811) by the more comprehensive smart-expansion
        version further down the file. Kept here so the queens + static
        defense pathway is callable, but it is not currently invoked from
        the canonical _prevent_resource_banking flow — wire in only after
        verifying it does not double-spend with the smart-expansion path.
        """
