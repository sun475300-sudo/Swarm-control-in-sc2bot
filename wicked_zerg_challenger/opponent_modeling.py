...
class OpponentModeling:
    ...
    def on_game_start(self, opponent_id: str, opponent_race=None):
        """게임 시작 시 호출 - 적 추적 시작 (legacy sync entry point)."""
    ...
    # NOTE: a buggy duplicate of on_step() previously lived here. It overrode
    # the proper async on_step() defined earlier in the class and crashed with
    # AttributeError because it referenced `self.current_opponent`, which was
    # never initialized in __init__. Removed so on_step() (line ~341) wins.

    def on_game_end(self, won: bool, lost: bool):
        """게임 종료 시 호출 - 데이터 저장 (legacy sync entry point)."""
