...
class OpponentModeling:
    ...
    def on_game_start(self, opponent_id: str, opponent_race=None):
        ...
        TODO(opponent-id-unification): two parallel identifier attrs exist
        — \`current_opponent\` (set here) and \`current_opponent_id\` (set
        by the older on_start hook above). \`_make_strategy_prediction\`
        guards on the latter, so prediction silently no-ops when only this
        lifecycle path is exercised. Pick one canonical attr."""
    ...
    async def on_step(self, iteration: int):
        """매 프레임 호출 — early-game signal detection plus continuous
        tracking (build order, timing attacks, tech progression) so that
        on_game_end has history to persist. Throttled by update_interval
        ...
        # Early-game signal detection + transition prediction
        if game_time <= 180.0:
        ...
        # Continuous tracking (data feeds on_game_end's saved history)
        ...
        # Publish to blackboard for downstream consumers
        if hasattr(self.bot, "blackboard") and self.bot.blackboard:
