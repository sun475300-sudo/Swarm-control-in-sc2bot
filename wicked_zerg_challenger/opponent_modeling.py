...
class OpponentModeling:
    ...
    async def on_step(self, iteration: int):
        ...
        Single canonical implementation. A second `on_step` further down
        the file used to silently shadow this richer one — losing build
        order tracking, timing-attack detection, tech progression and the
        blackboard updates. Merged its `current_opponent` guard in here
        so the older callers (which only ever set `current_opponent_id`)
