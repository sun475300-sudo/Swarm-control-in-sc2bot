...
class OpponentModeling:
    ...
    # NOTE: a duplicate ``on_step`` used to live here and shadowed the proper
    # implementation defined earlier in the class. It also referenced
    # ``self.current_opponent_id`` which is never assigned, so any caller that
    # reached it would crash with AttributeError. Removed in favor of the
    # canonical ``on_step`` defined above.
