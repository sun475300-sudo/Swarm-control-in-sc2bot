...
class OpponentModeling:
    ...
    def current_opponent(self) -> Optional[str]:
        """Backwards-compatible alias for current_opponent_id.

        Older callers (wicked_zerg_bot_pro_impl, integration tests) read/write
        ``current_opponent``. Newer code uses ``current_opponent_id``. Keep
        both in sync via this property.
        """
    ...
    # NOTE: a second ``async def on_step`` used to live here and silently
    # shadowed the comprehensive on_step defined above (early-signal +
    # build-order + timing-attack + tech tracking + blackboard updates).
    # Removed; the canonical on_step at the top of the class now runs.
