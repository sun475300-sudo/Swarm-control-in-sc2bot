...
- **위치**: `intel_manager.py` 라인 193-196, 563-575
...
# 구현: 최대 50개 제한 로직 추가
if len(self.enemy_last_positions) >= self.MAX_ENEMY_TRACKING:
    # 오래된 항목 제거
    oldest_tags = sorted(...)
    for tag, _ in oldest_tags:
...
### 리포트
- ? `PERFORMANCE_OPTIMIZATION_APPLIED.md` - 적용 보고서
- ? `PERFORMANCE_OPTIMIZATION_COMPLETE.md` - 완료 보고서
