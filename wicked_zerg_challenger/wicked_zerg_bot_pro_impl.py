    ...
    async def on_step(self, iteration: int):
        ...
        if self.scoring_system:
            try:
                ...
                if iteration % 220 == 0:
                    self.logger.warning("ScoringSystem.on_step failed: %s", e)
        ...
        if self.awareness_engine:
            try:
                ...
                if iteration % 220 == 0:
                    self.logger.warning("AwarenessEngine.on_step failed: %s", e)
    ...
    async def on_end(self, game_result):
        ...
        if self.awareness_engine:
            try:
                ...
                self.logger.warning("AwarenessEngine summary failed: %s", e)
