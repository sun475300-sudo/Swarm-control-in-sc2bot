...
class ComprehensiveUnitAbilities:
                ...
                if injured:
                    ...
                    if AbilityId.TRANSFUSION_TRANSFUSION in abilities:
                        if self._can_use_ability(
                            queen.tag, "transfusion", game_time, self.TRANSFUSION_CD
                        ):
                            self.bot.do(
            ...
            if queen.energy >= 25 and self.bot.townhalls.exists:
                if len(self.bot.structures(UnitTypeId.CREEPTUMORBURROWED)) < 20:
                    ...
                    if queen.distance_to(closest_base) < 15:
                        # 크립 확장 위치 찾기
                        position = closest_base.position.towards(
            ...
            if infestor.energy >= 100:
                ...
                if high_value:
                    ...
                    if AbilityId.NEURALPARASITE_NEURALPARASITE in abilities:
                        if self._can_use_ability(
                            infestor.tag, "neural", game_time, self.NEURAL_CD
                        ):
                            self.bot.do(
                                infestor(
                ...
                for enemy in enemies:
                    ...
                    if nearby.amount >= 5:  # 5명 이상 밀집
                        ...
                        if AbilityId.FUNGALGROWTH_FUNGALGROWTH in abilities:
                            if self._can_use_ability(
                                infestor.tag, "fungal", game_time, self.FUNGAL_CD
                            ):
                                self.bot.do(
                                    infestor(
    ...
    async def _viper_abilities(self, game_time: float):
        ...
        for viper in vipers:
            ...
            if viper.energy >= 125:
                air_enemies = self.bot.enemy_units.filter(
            ...
            if viper.energy >= 75:
                ...
                if high_value:
                    ...
                    if AbilityId.EFFECT_ABDUCT in abilities:
                        if self._can_use_ability(
                            viper.tag, "abduct", game_time, self.ABDUCT_CD
                        ):
                ...
                if ranged_enemies.amount >= 6:
                    ...
                    if AbilityId.BLINDINGCLOUD_BLINDINGCLOUD in abilities:
                        if self._can_use_ability(
                            ...
                            "blinding_cloud",
                            game_time,
                            self.BLINDING_CLOUD_CD,
                        ):
                            center = ranged_enemies.center
                            self.bot.do(
                                viper(AbilityId.BLINDINGCLOUD_BLINDINGCLOUD, center)
                            )
            ...
            if overseer.energy >= 75:
                ...
                if production:
                    ...
                    if AbilityId.CONTAMINATE_CONTAMINATE in abilities:
                        if self._can_use_ability(
                            overseer.tag, "contaminate", game_time, self.CONTAMINATE_CD
                        ):
                            self.bot.do(
    ...
    async def _corruptor_abilities(self, game_time: float):
        ...
        corruptors = self.bot.units(UnitTypeId.CORRUPTOR).filter(
            lambda c: c.energy >= 75
        )
        if not corruptors:
            return
        ...
        for corruptor in corruptors:
            ...
            if enemy_structures:
                ...
                if AbilityId.CAUSTICSPRAY_CAUSTICSPRAY in abilities:
                    if self._can_use_ability(
                        corruptor.tag, "caustic_spray", game_time, self.CAUSTIC_SPRAY_CD
                    ):
                        self.bot.do(
                            corruptor(AbilityId.CAUSTICSPRAY_CAUSTICSPRAY, target)
                        )
        ...
        for host in swarmhosts:
            ...
            if enemies:
                ...
                if AbilityId.EFFECT_SPAWNLOCUSTS in abilities:
                    if self._can_use_ability(
                        host.tag, "locust", game_time, self.LOCUST_CD
                    ):
    ...
    async def _tactical_burrow(self, game_time: float):
            ...
            if enemies_nearby or enemy_structures_nearby:
                ...
                if lurker.tag not in self.burrowed_units:
                    ...
                    if AbilityId.BURROWDOWN_LURKER in abilities:
                        ...
                        self.logger.info(
                            f"[LURKER] Burrowing before attack at {lurker.position}"
                        )

        # === 2. INFESTOR: 적이 가까우면 잠복 (은폐) ===
        infestors = self.bot.units(UnitTypeId.INFESTOR).filter(
            lambda i: not i.is_burrowed and i.energy >= 50
        )
        for infestor in infestors:
            # 적이 10거리 내에 있으면 잠복
        ...
        swarmhosts = self.bot.units(UnitTypeId.SWARMHOSTMP).filter(
            lambda s: not s.is_burrowed
        )
        for host in swarmhosts:
    ...
    async def _overlord_creep_harass(self, game_time: float):
        ...
        # Lair 업그레이드가 필요
        if (
            not self.bot.structures(UnitTypeId.LAIR).ready
            and not self.bot.structures(UnitTypeId.HIVE).ready
        ):
            return

        overlords = self.bot.units(
            UnitTypeId.OVERLORDTRANSPORT
        )  # Generate Creep 가능한 대군주
        if not overlords:
            return
    ...
    # HELPER FUNCTIONS
    # ========================================
    def _can_use_ability(
        self, unit_tag: int, ability_name: str, game_time: float, cooldown: float
    ) -> bool:
        """스킬 쿨다운 확인"""
