...
if adaptive_lr_path.exists():
    ...
    logger.info("[OK] 최적 학습률: {data['best_learning_rate']:.6f}")
    logger.info("[OK] 최고 승률: {data['best_win_rate']:.2%}")
    logger.info(f"[OK] 총 게임 수: {data['total_games']}")
    logger.info(f"[OK] 총 승리 수: {data['total_wins']}")
    if data["total_games"] > 0:
        logger.info("[OK] 전체 승률: {data['total_wins']/data['total_games']:.2%}")
    else:
    ...
    if data.get("adjustment_history"):
        ...
        for adj in data["adjustment_history"][-5:]:
            ...
            logger.info(
                "  {action_mark} Game {adj['game']}: {adj['old_lr']:.6f} -> {adj['new_lr']:.6f} ({adj['action']})"
            )
else:
...
if model_path.exists():
    ...
    logger.info("[OK] 베이스라인: {float(data['baseline'][0]):.4f}")
    logger.info("\n[MODEL] 모델 파라미터:")
...
if analytics_path.exists():
    ...
    for race, stats in data["race_stats"].items():
        if stats["games"] > 0:
            ...
            logger.info(
                "  vs {race}: {stats['wins']}/{stats['games']}승 ({race_wr:.1f}%)"
            )
else:
...
if adaptive_lr_path.exists():
    ...
    logger.info(
        "[BEST] 최고 성과: {data['best_win_rate']:.2%} (LR: {data['best_learning_rate']:.6f})"
    )
