...
class EconomyCombatBalancer:
    ...
    def apply_threat_level(self, threat_level: str) -> int:
        """Adjust economy targets from live combat threat."""
        level = str(threat_level or "low").lower()
        if level not in self.threat_drone_targets:
            level = "low"
        self.current_threat_level = level
        target = self.threat_drone_targets[level]
        if target > 0:
            self.drone_targets["late"] = min(self.base_drone_targets["late"], target)
            self.drone_targets["mid"] = min(self.base_drone_targets["mid"], target)
        return target
