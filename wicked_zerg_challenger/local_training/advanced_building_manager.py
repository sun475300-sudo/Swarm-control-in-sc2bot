...
class AdvancedBuildingManager:
    ...
    def __init__(self, bot):
        ...
        # * 끼인 일꾼 감지용 위치 기록 (moving-but-stuck 케이스) *
        self._worker_position_history: Dict[int, Tuple[float, float]] = {}
        self._worker_stuck_counter: Dict[int, int] = {}
        self.stuck_position_epsilon = 0.15  # 이 거리 미만 이동은 "정지"로 간주
        self.stuck_check_threshold = 3  # 연속 N회 정지 관측 시 끼인 것으로 판단
        ...
        if distance > 0:
            ...
            for ratio in [0.3, 0.5, 0.7]:
                chokepoint = base_position + Point2(
                    (direction.x * ratio, direction.y * ratio)
                )
    ...
    async def build_defense_building_at_chokepoint(
        ...
        try:
            ...
            if result:
                logger.info(
                    f"Built {building_type} at chokepoint "
            ...
            if self.bot.townhalls.exists:
                ...
                for building_type in [UnitTypeId.SPINECRAWLER, UnitTypeId.SPORECRAWLER]:
                    if self.bot.can_afford(building_type):
                        ...
                        if await self.build_with_worker_safety(
                            building_type, target_pos
                        ):
    ...
    def has_resource_surplus(self) -> Tuple[bool, float, float]:
        ...
        has_surplus = (
            minerals >= self.resource_surplus_threshold
            or gas >= self.gas_surplus_threshold
        )
    ...
    async def build_tech_buildings_aggressively(self) -> Dict[UnitTypeId, bool]:
        ...
        if gas_surplus > 200:
            ...
            if (
                not self.bot.structures(UnitTypeId.LURKERDENMP).exists
            ...
            if (
                self.bot.structures(UnitTypeId.HIVE).ready.exists
                and not self.bot.structures(UnitTypeId.ULTRALISKCAVERN).exists
            ...
            if (
                self.bot.structures(UnitTypeId.HIVE).ready.exists
                and spires.exists
                and not self.bot.structures(UnitTypeId.GREATERSPIRE).exists
                and self.bot.already_pending(UnitTypeId.GREATERSPIRE) == 0
            ):
        ...
        if mineral_surplus > 500:
            ...
            if (
                ...
                and not self.bot.structures(UnitTypeId.BANELINGNEST).exists
        ...
        for th in self.bot.townhalls:
            ...
            if hasattr(self.bot, "mineral_field"):
                for mineral in self.bot.mineral_field.closer_than(12, th_pos):
                    if self._is_on_line_segment(
                        position, th_pos, mineral.position, self.worker_path_width
                    ):
                        return True
            ...
            if hasattr(self.bot, "vespene_geyser"):
                for gas in self.bot.vespene_geyser.closer_than(12, th_pos):
                    if self._is_on_line_segment(
                        position, th_pos, gas.position, self.worker_path_width
                    ):
                        return True
    ...
    def _is_on_line_segment(
        self, point: Point2, line_start: Point2, line_end: Point2, width: float
    ) -> bool:
        """
        점이 선분 근처에 있는지 확인.
        ...
        try:
            ...
            distance = math.sqrt(
                (point.x - proj_point.x) ** 2 + (point.y - proj_point.y) ** 2
            )
    ...
    def get_safe_building_position(
        self, building_type: UnitTypeId, near: Point2, avoid_worker_paths: bool = True
    ) -> Optional[Point2]:
        """
    ...
    def _is_safe_position(
        self, position: Point2, building_type: UnitTypeId, avoid_worker_paths: bool
    ) -> bool:
        """건설 위치가 안전한지 확인."""
        try:
    ...
    async def build_with_worker_safety(
        self, building_type: UnitTypeId, near: Point2
    ) -> bool:
        ...
        if self.placement_helper:
            success = await self.placement_helper.build_structure_safely(
                building_type, near, max_distance=15.0
            )
            if success:
    ...
    async def rescue_stuck_workers(self) -> int:
        ...
        for worker in self.bot.workers:
            try:
                ...
                # 2. 움직이지만 제자리인 경우 (건물 사이에 끼여 이동 명령이
                # 계속 실패하는 경우) - 위치 기록을 비교해 연속 정지 감지
                if not is_stuck and hasattr(worker, "position"):
                    pos = (worker.position.x, worker.position.y)
                    last_pos = self._worker_position_history.get(worker.tag)
                    if last_pos is not None:
                        moved = (
                            (pos[0] - last_pos[0]) ** 2 + (pos[1] - last_pos[1]) ** 2
                        ) ** 0.5
                        if moved < self.stuck_position_epsilon:
                            self._worker_stuck_counter[worker.tag] = (
                                self._worker_stuck_counter.get(worker.tag, 0) + 1
                            )
                        else:
                            self._worker_stuck_counter[worker.tag] = 0
                    self._worker_position_history[worker.tag] = pos

                    if (
                        self._worker_stuck_counter.get(worker.tag, 0)
                        >= self.stuck_check_threshold
                    ):
                        is_stuck = True
                ...
                if is_stuck:
                    if hasattr(self.bot, "structures"):
                        ...
                        if (
                            nearby_structures.amount >= 2
                        ):  # 2개 이상 건물 근처면 낀 것으로 간주
                            if (
                                hasattr(self.bot, "mineral_field")
                                and self.bot.mineral_field.exists
                            ):
                                closest_mineral = self.bot.mineral_field.closest_to(
                                    worker
                                )
        ...
        # 죽거나 사라진 일꾼의 기록 정리 (메모리 누적 방지)
        stale_tags = set(self._worker_position_history) - seen_tags
        for tag in stale_tags:
            self._worker_position_history.pop(tag, None)
            self._worker_stuck_counter.pop(tag, None)
