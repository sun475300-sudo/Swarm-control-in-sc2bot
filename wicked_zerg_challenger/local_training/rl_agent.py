...
class PolicyNetwork:
    ...
    def update_weights(
        self, learning_rate: float = 0.001, max_grad_norm: float = 5.0
    ) -> None:
...
class RLAgent:
    ...
    def __init__(
        ...
        self.model_path = (
            Path(model_path)
            if model_path
            else Path("local_training/models/rl_agent_model.npz")
        )
    ...
    def get_action(
        ...
        if len(state) < self.policy.input_dim:
            state = np.concatenate(
                [state, np.zeros(self.policy.input_dim - len(state))]
            )
    ...
    def calculate_intermediate_rewards(
        self, base_count: int, army_supply: float, supply_blocked: bool
    ) -> float:
        """
    ...
    def end_episode(
        ...
        if min_len < len(self.states):
            logger.info(
                f"Dimension mismatch detected: states={len(self.states)}, "
        ...
        self.baseline = (
            self.baseline_decay * self.baseline + (1 - self.baseline_decay) * avg_return
        )
        ...
        for cache, action, advantage in zip(self.caches, self.actions, advantages):
            ...
            total_loss += (
                -np.log(probs[action] + 1e-9) * advantage - entropy_coeff * entropy
            )
        ...
        if len(self.training_history) >= 20:
            ...
            older_20 = (
                [h.get("avg_reward", 0) for h in self.training_history[-40:-20]]
                if len(self.training_history) >= 40
                else recent_20
            )
    ...
    def save_experience_data(self, path: str) -> bool:
        ...
        try:
            ...
            # os.replace()는 POSIX/Windows 모두에서 원자적으로 기존 파일을 덮어쓴다.
            # (기존 파일을 먼저 remove() 하면 그 사이 크래시/디스크풀 시
            #  원본과 신규 파일이 모두 사라지는 데이터 유실 창이 생김)
    ...
    def save_model(self, path: Optional[str] = None) -> bool:
        ...
        # with it. A tmp name of "model.tmp" therefore silently becomes
        # "model.tmp.npz" on disk, the tmp_path.exists() check below never
        # fires, and save_path is never actually replaced. Keeping ".npz" on
        # the tmp name avoids the double-extension mismatch.
        ...
        try:
            ...
            # os.replace() atomically overwrites save_path on both POSIX and
            # Windows in one syscall. Unlinking save_path first (the previous
            # approach) opens a window where a crash/disk-full between the
            # unlink and the move destroys the old model with no new one in
            # its place.
            if tmp_path.exists():
    ...
    def is_ready_for_deployment(self) -> Tuple[bool, str]:
        ...
        if self.episode_count < min_games:
            return (
                False,
                f"Not enough training games ({self.episode_count}/{min_games})",
            )
