...
class RLAgent:
    ...
    def save_experience_data(self, path: str) -> bool:
        ...
        try:
            ...
            # os.replace()는 remove 후 rename하는 방식과 달리 rename 실패 시에도
            # 원본 파일을 보존한다 (기존 방식은 그 사이 창구에서 데이터 손실 가능).
    ...
    def save_model(self, path: Optional[str] = None) -> bool:
        ...
        # np.savez() auto-appends ".npz" to any path not already ending in it, so a
        # bare ".tmp" suffix here meant this code checked/replaced a path numpy never
        # wrote to, silently no-op'ing the save while still returning True.
        ...
        try:
            ...
            # os.replace() is atomic on POSIX/Windows and never deletes the destination
            # before the replacement lands, unlike the prior unlink()-then-move().
            if tmp_path.exists():
