...
class RLAgent:
    ...
    def save_experience_data(self, path: str) -> bool:
        ...
        try:
            ...
            # os.replace()는 대상 파일이 존재해도 POSIX/Windows 모두에서 원자적으로
            # 덮어쓴다. 이전에는 os.remove() 후 os.rename()을 호출했는데, 그 사이에
            # rename이 실패하면(디스크풀/권한 오류 등) 기존 파일도 사라지고 새 파일도
            # 생기지 않아 데이터가 완전히 유실되는 문제가 있었다.
