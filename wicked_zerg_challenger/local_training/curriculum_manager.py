...
class CurriculumManager:
    ...
    def record_win(self, opponent_race: str = None) -> bool:
        ...
        if opponent_race and opponent_race in self.race_stats:
            ...
            logger.info(
                "vs {opponent_race}: {race_wins}W/{race_games}G ({race_rate:.1f}%)"
            )
    ...
    def check_promotion(self, win_rate: float, total_games: int) -> bool:
        ...
        if (
            ...
            if 0 <= new_idx < len(self.levels):
                ...
                logger.info(f"  {old_difficulty} -> {new_difficulty}")
                logger.info(
                    "  Win Rate: {win_rate*100:.1f}% (threshold: {self.promotion_threshold*100}%)"
                )
    ...
    def check_demotion(self, win_rate: float, total_games: int) -> bool:
        ...
        if win_rate < self.demotion_threshold and self.current_idx > 0:
            ...
            if 0 <= new_idx < len(self.levels):
                ...
                logger.info(f"  {old_difficulty} -> {new_difficulty}")
                logger.info(
                    "  Win Rate: {win_rate*100:.1f}% (threshold: {self.demotion_threshold*100}%)"
                )
                logger.info(f"  Games at previous level: {total_games}")
                logger.info(f"{'='*70}\n")
    ...
    def print_race_stats(self):
        ...
        logger.info(f"{'='*70}")
        logger.info("{'종족':<10} {'승리':<8} {'패배':<8} {'게임':<8} {'승률':<10}")
        logger.info(f"{'-'*70}")

        for race in ["Terran", "Protoss", "Zerg"]:
            r = stats[race]
            logger.info(
                "{race:<10} {r['wins']:<8} {r['losses']:<8} {r['games']:<8} {r['win_rate']:.2f}%"
            )

        logger.info(f"{'-'*70}")
        t = stats["total"]
        logger.info(
            "{'전체':<10} {t['wins']:<8} {t['losses']:<8} {t['games']:<8} {t['win_rate']:.2f}%"
        )
        logger.info(f"{'='*70}\n")
