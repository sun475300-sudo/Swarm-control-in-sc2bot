...
class ProductionResilience:
            ...
            async def _build_with_reservation(structure_type, *args, **kwargs):
                ...
                if structure_type in unique_tech_buildings:
                    try:
                        ...
                        logger.debug(f"__init__ swallow: {e!r}")
    ...
    def _cleanup_build_reservations(self) -> None:
        ...
        try:
            ...
            logger.debug(f"_cleanup_build_reservations swallow: {e!r}")
            ...
            if pending_hatcheries == 0 and bases < 5:
                if b.can_afford(UnitTypeId.HATCHERY):
                    try:
                        ...
                        logger.debug(f"fix_production_bottleneck swallow: {e!r}")
            ...
            if (
                ...
                if b.townhalls.exists and len(b.townhalls) < 3:
                    try:
                        ...
                        logger.debug(f"_force_emergency_production swallow: {e!r}")
            ...
            if iteration % 50 == 0:
                ...
                if spawning_pool_query.ready.exists:
                    ...
                    try:
                        ...
                        logger.debug(f"diagnose_production_status swallow: {e!r}")
                ...
                if max_deficit_unit == UnitTypeId.HYDRALISK:
                    ...
                    if roaches_ready.exists and b.can_afford(
                        ...
                        try:
                            ...
                            logger.debug(f"build_army_aggressive swallow: {e!r}")
    ...
    async def force_resource_dump(self) -> None:
        ...
        if (
            ...
            try:
                ...
                logger.debug(f"force_resource_dump swallow: {e!r}")
                ...
                if tech_coordinator:
                    ...
                    if b.townhalls.exists:
                        try:
                            ...
                            logger.debug(f"build_terran_counters swallow: {e!r}")
        # NOTE: Roach Warren building is now handled by _auto_build_tech_structures()
        # Removed duplicate code to prevent building spam
            ...
            if (
                ...
                if b.townhalls.exists:
                    try:
                        ...
                        logger.debug(f"build_zerg_counters swallow: {e!r}")
            ...
            if ling.is_idle:
                ...
                if targets:
                    try:
                        ...
                        logger.debug(f"_manage_existing_scouts swallow: {e!r}")
        ...
        if not larvae.exists:
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(f"_spend_excess_gas swallow: {e!r}")
            return
        ...
        if (
            ...
            if b.can_afford(UnitTypeId.HYDRALISKDEN) and b.townhalls.exists:
                try:
                    ...
                    logger.debug(f"_build_gas_heavy_tech swallow: {e!r}")
