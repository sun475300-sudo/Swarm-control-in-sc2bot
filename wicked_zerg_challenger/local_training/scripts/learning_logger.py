...
class LearningLogger:
    ...
    def get_learning_summary(
            self, replay_path: Optional[Path] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if replay_path:
                logs = [log for log in logs if log.get(
                    "replay") == replay_path.name]

            total_strategies = sum(log.get("strategy_count", 0)
                                   for log in logs)
...
def main():
    ...
    parser.add_argument(
        "--log-file",
        type=str,
        default="D:/replays/learning_log.txt",
        help="Log file path")
    parser.add_argument(
        "--summary",
        action="store_true",
