...
STRATEGY_MAP = {"ECONOMY": 0, "AGGRESSIVE": 1, "DEFENSIVE": 2, "TECH": 3, "ALL_IN": 4}
...
def train_from_replays(replay_dir, model_path=None):
    ...
    for r_path in replay_files:
        try:
            ...
            for event in replay.events:
                ...
                if (
                    hasattr(event, "second")
                    and event.second > 0
                    and event.second % 30 == 0
                ):
            ...
            stats_events = [
                ...
                if "PlayerStatsEvent" in type(e).__name__ and e.player.pid == winner.pid
            ]
    ...
    if experiences:
        ...
        try:
            ...
            logger.info(
                f"Moving {len(processed_replays)} processed replays to {completed_dir}..."
            )
