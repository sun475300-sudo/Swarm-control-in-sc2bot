...
class ReplayBuildOrderExtractor:
        ...
        if replay_dir is None:
            ...
            if not replay_dir or not os.path.exists(replay_dir):
                ...
                possible_paths = [
                    ...
                    # All Zerg pro gamer replays (highest priority)
    ...
    def extract_build_order(
            self,
            replay_path: Path,
            phase_focus: Optional[Dict] = None) -> Optional[Dict]:
        """Extract build order from a single replay file"""
        if not SC2READER_AVAILABLE:
            logger.error(
                "sc2reader not available. Cannot extract build orders.")
            return None
        ...
        try:
            ...
            # Always use play_race attribute from replay metadata (most
            # reliable)
            zerg_player = None
            pro_zerg_player = None

            # IMPROVED: First pass - find Zerg players by race (strict
            # validation)
            for player in replay.players:
                ...
                if hasattr(player, 'play_race'):
                    ...
                    if player_race == "zerg":
                        ...
                        if any(
                                pro_name in player_name_lower for pro_name in PRO_ZERG_PLAYERS):
                            pro_zerg_player = player
                            break
                        elif not zerg_player:
                            zerg_player = player

            # IMPROVED: Second pass - if no Zerg found by race, check
            # alternative methods
            if not zerg_player and not pro_zerg_player:
                # Fallback: Check if any player has Zerg-related indicators
                # But this should be rare - most replays have proper race
                # metadata
                for player in replay.players:
                    if hasattr(player, 'name'):
                        player_name_lower = (player.name or "").lower()
                        # Only use name-based detection if race metadata is
                        # missing
                        if not hasattr(
                                player, 'play_race') or player.play_race is None:
                            if any(
                                zerg_keyword in player_name_lower for zerg_keyword in [
                                    "zerg", "z", "zv"]):
                                logger.warning(
            ...
            if not zerg_player:
                logger.debug(
                    f"No Zerg player found in {replay_path.name} (strict validation)")
                return None
            ...
            build_order = {
                ...
                "map_name": replay.map_name if hasattr(
                    replay,
                    'map_name') else "Unknown",
                "game_length": replay.game_length.seconds if hasattr(
                    replay,
                    'game_length') else 0,
                "timings": {}}
            ...
            # CRITICAL: Track supply history to filter out cancellations and
            # unit losses
            supply_history: Dict[float, int] = {}  # time -> supply
            # (unit_name, time, supply)
            unit_creation_events: List[Tuple[str, float, int]] = []

            if hasattr(replay, 'tracker_events'):
                for event in replay.tracker_events:
                    try:
                        if hasattr(event,
                                   'player') and event.player == zerg_player:
                            # Track supply history for cancellation/loss
                            # detection
                            if hasattr(
                                    event, 'second') and hasattr(
                                    event, 'food_used'):
                            ...
                            if hasattr(
                                ...
                                if hasattr(unit, 'name'):
                                    ...
                                    if unit_name in unit_to_parameter:
                                        ...
                                        supply = self._get_supply_at_time(
                                            replay, zerg_player, event.second)
                                        if supply > 0:
                                            unit_creation_events.append(
                                                (param_name, event.second, supply))
                        ...
                        if len(unit_creation_events) == 0 and len(
                                supply_history) < 10:
            ...
            for param_name, creation_time, creation_supply in unit_creation_events:
                if param_name not in build_order["timings"]:
                    ...
                    is_valid = True
                    check_times = [t for t in supply_history.keys(
                    ) if creation_time <= t <= creation_time + 10.0]
                    if check_times:
                        max_supply_after = max(
                            supply_history[t] for t in check_times)
            ...
            # UnitInitEvent also uses control_pid and is used for some
            # structures like Extractor
            ...
            # Extractor often appears in tracker_events as UnitInitEvent, not
            # in events as UnitBornEvent
            all_events_to_check = []

            # Add UnitBornEvents from replay.events
            for event in replay.events:
                if hasattr(
                        event, '__class__') and 'UnitBorn' in str(
                        event.__class__):
                    all_events_to_check.append(event)

            # Add UnitInitEvents from tracker_events (for Extractor and other
            # structures)
            if hasattr(replay, 'tracker_events'):
                for event in replay.tracker_events:
                    if hasattr(
                            event, '__class__') and 'UnitInit' in str(
                            event.__class__):
                        all_events_to_check.append(event)

            for event in all_events_to_check:
                try:
                    # Check if this is a UnitBornEvent or UnitInitEvent for
                    # Zerg player
                    is_zerg_event = False
                    event_class_str = str(event.__class__)
                    if 'UnitBorn' in event_class_str or 'UnitInit' in event_class_str:
                        if zerg_pid is not None and hasattr(
                                event, 'control_pid'):
                    ...
                    if is_zerg_event:
                        ...
                        if hasattr(event, 'unit'):
                            ...
                            if hasattr(unit, 'is_building'):
                                ...
                                building_keywords = [
                                    ...
                                    'Infestation']
                                is_building = any(
                                    kw in unit_name for kw in building_keywords)
                            # CRITICAL: For Extractor, also check if it's in
                            # our mapping (UnitInitEvent may not have
                            # is_building)
                            if not is_building and unit_name and unit_name in unit_to_parameter:
                                # If it's in our mapping, it's definitely a
                                # building we care about
                                is_building = True
                        ...
                        if is_building and unit_name and unit_name in unit_to_parameter and event_time is not None:
                            ...
                            # Special handling for Hatchery: skip the first one
                            # (starting base)
                            ...
                            if param_name not in build_order["timings"]:
                                # Get supply at time of event
                                supply = self._get_supply_at_time(
                                    replay, zerg_player, event_time)

                                # CRITICAL: If supply lookup fails, try to estimate from nearby events
                                # This is especially important for early game
                                # events like Extractor
                                if supply <= 0:
                                    # Try to find supply from nearby tracker
                                    # events (within 5 seconds)
                                    if hasattr(replay, 'tracker_events'):
                                        for tracker_event in replay.tracker_events:
                                            try:
                                                if hasattr(
                                                        tracker_event, 'player') and tracker_event.player == zerg_player:
                                                    if hasattr(
                                                            tracker_event, 'second') and hasattr(
                                                            tracker_event, 'food_used'):
                                                        time_diff = abs(
                                                            tracker_event.second - event_time)
                                ...
                                # Extractor is typically built around 15-20
                                # supply, so use that as fallback
                                if supply <= 0 and 'Extractor' in unit_name:
                                    # Estimate supply based on game time (rough
                                    # approximation)
            ...
            logger.error(
                f"Error extracting build order from {replay_path.name}: {e}")
            return None
            ...
            try:
                if hasattr(replay, 'tracker_events'):
                    ...
                    for event in replay.tracker_events:
                        if hasattr(event, 'player') and event.player == player:
                            if hasattr(
                                    event, 'second') and abs(
                                    event.second - time_seconds) < 2.0:
                                if hasattr(
                                        event, 'unit') and hasattr(
                                        event.unit, 'supply_cost'):
    ...
    def _extract_strategies(
        ...
        try:
                    ...
                    for event in replay.events:
                        if hasattr(event,
                            ...
                            if hasattr(event, 'unit_type_name'):
                                ...
                                if any(
                                    spell_unit in unit_name for spell_unit in [
                                        "viper", "infestor", "swarmhost"]):
                                    timing = getattr(event, 'second', 0)
                                    strategy_id = strategy_db.add_strategy(
            ...
            logger.warning(
                f"Failed to extract strategies from {replay_path.name}: {e}")
    ...
    def learn_from_replays(self, max_replays: int = 100) -> Dict[str, float]:
        ...
        try:
            ...
            if ReplayLearningTracker:
                tracker = ReplayLearningTracker(
                    tracking_file, min_iterations=5)
            else:
                tracker = None
            ...
            if tracker:
                replay_files = tracker.get_replays_for_training(
                    self.replay_dir, completed_dir)
                if not replay_files:
                    logger.warning(
                        "No replay files found that need training. All replays may be completed.")
                    return {}
                ...
                if not replay_files:
                    logger.warning(
                        "No replay files found. Cannot learn build orders.")
                    return {}
                replay_files = replay_files[:max_replays]
        ...
        try:
            ...
            crash_handler.recover_stale_sessions(
                max_age_seconds=1800)  # 30 minutes
        except (ImportError, SyntaxError, IndentationError) as e:
            logger.warning(
                f"replay_crash_handler not available - crash recovery disabled: {e}")
        ...
        for replay_path in replay_files:
            ...
            if crash_handler and crash_handler.is_bad_replay(replay_path):
                ...
                logger.warning(
                    f"[BAD REPLAY] {replay_path.name} - Skipping (crashed {crash_count} times)")
                continue
            ...
            try:
                ...
                build_order = self.extract_build_order(
                    replay_path, phase_focus=phase_info)
                ...
                if build_order:
                    ...
                    try:
                        ...
                        status_manager = LearningStatusManager(
                            status_file, min_iterations=5)
                        status_count = status_manager.increment_learning_count(
                            replay_path)
                        logger.info(
                            f"  [STATUS TRACKER] {replay_path.name}: {status_count}/{status_manager.min_iterations} iterations")
                    ...
                    if tracker and tracker.is_completed(replay_path):
                        if tracker.move_completed_replay(
                            ...
                            logger.info(
                                f"  [COMPLETED] Moved {replay_path.name} to completed folder ({new_count} iterations)")
                        else:
                            logger.info(
                                f"  [IN PROGRESS] {replay_path.name} needs {tracker.min_iterations - new_count} more iterations")
                ...
                if crash_handler:
                    ...
                    logger.error(
                        f"[CRASH] {replay_path.name} - Learning failed (crash count: {crash_count}): {e}")
                else:
                    logger.error(
                        f"[ERROR] {replay_path.name} - Learning failed: {e}")
                continue

            # Collect timing statistics (moved outside try-except to ensure it
            # runs)
            if build_order and "timings" in build_order:
                for param_name, timing_data in build_order["timings"].items():
                    if "supply" in timing_data:
                        self.timing_stats[param_name].append(
                            timing_data["supply"])
        ...
        for param_name, supply_values in self.timing_stats.items():
            if supply_values:
                ...
                logger.info(
                    f"Learned {param_name}: {median_supply:.1f} supply (from {len(supply_values)} samples)")

        return learned_params

    def save_learned_parameters(
            self, learned_params: Dict[str, float], output_file: str = None):
        """
        ...
        data = {
            ...
            # Save first 10 for reference
            "build_orders": self.build_orders[:10]
        }
...
def update_config_with_learned_params(learned_params: Dict[str, float]):
    ...
    # Path structure: scripts/ -> local_training/ -> wicked_zerg_challenger/
    # (project root)
    script_path = Path(__file__).resolve()
    possible_config_paths = [
        script_path.parents[2] / "config.py",
    ...
    logger.info(
        "Learned parameters (update config.py manually or use JSON file):")
    ...
    logger.info(
        f"Saved learned parameters to {learned_json_path} (local copy for config.py)")
    logger.info(
        "config.py will read from learned_build_orders.json automatically")
...
def main():
    ...
    if not default_replay_dir or not os.path.exists(default_replay_dir):
        ...
        possible_paths = [
            Path("D:/replays/replays"),
            # Training source directory (highest priority)
            Path(__file__).parent.parent / "replays_archive",
            Path.home() / "replays" / "replays",
            Path.home() / "replays",
            Path("replays_archive"),
        ...
        for path in possible_paths:
            ...
            # Default to training source directory
    ...
    print("=" * 70)
    print(
        f"Learned {len(learned_params)} parameters from {len(extractor.build_orders)} replays")
    print("\nLearned Parameters:")
    for param_name, value in learned_params.items():
    ...
    # 3. learned_build_orders.json (in local_training/scripts/) - via
    ...
    try:
        ...
        if auditor.pro_data:
            print(
                f"[🧠 STRATEGY AUDIT] Loaded pro gamer data: {len(auditor.pro_data.get('build_orders', []))} build orders")
            print("[🧠 STRATEGY AUDIT] Strategy audit ready for game analysis")
        else:
            print(
                "[🧠 STRATEGY AUDIT] Warning: Pro gamer data not loaded. Analysis may be limited.")
