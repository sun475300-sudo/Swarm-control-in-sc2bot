...
class StrategyDatabase:
    ...
    def _load_database(self) -> Dict[str, StrategyEntry]:
        ...
        try:
            ...
            for strategy_id, entry_data in data.items():
                try:
                    strategies[strategy_id] = StrategyEntry.from_dict(
                        entry_data)
                except Exception as e:
                    print(
                        f"[WARNING] Failed to load strategy {strategy_id}: {e}")
            return strategies
 ...
 List of matching strategy entries
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for entry in self.strategies.values():
            by_type[entry.strategy_type] = by_type.get(
                entry.strategy_type, 0) + 1
...
def main():
    ...
    parser.add_argument(
        "--db-path",
        type=str,
        default="D:/replays/strategy_db.json",
        help="Database path")
    parser.add_argument(
        "--list",
        action="store_true",
