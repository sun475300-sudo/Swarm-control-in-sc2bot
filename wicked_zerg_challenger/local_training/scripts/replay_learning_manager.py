...
class ReplayLearningTracker:
 ...
 Dictionary with focus areas and weights
    ...
    def _load_tracking(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            # Support both old format (count only) and new format (full
            # metadata)
    ...
    def _get_replay_hash(self, replay_path: Path) -> str:
        ...
        try:
            # Use file size + modification time as hash (faster than full file
            # hash)
    ...
    def get_replays_for_training(
            self,
            replay_dir: Path,
            completed_dir: Path) -> List[Path]:
        """
 ...
 List of replay paths that need more training iterations
    ...
    def move_completed_replay(
            self,
            replay_path: Path,
            completed_dir: Path) -> bool:
        ...
        try:
            ...
            print(
                f"[ERROR] Failed to move completed replay {replay_path.name}: {e}")
            return False
...
def main():
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="List replay learning status")
    ...
    if args.list:
        ...
        completed_count = sum(
            1 for rp in all_replays if tracker.is_completed(rp))
        print(
            f"Completed: {completed_count}, Pending: {len(all_replays) - completed_count}")
