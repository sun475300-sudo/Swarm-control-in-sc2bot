...
class LearningStatusManager:
 ...
 This is a hard requirement - files with < 5 iterations MUST NOT be moved/deleted
        ...
        if count < self.min_iterations:
            print(
                f"[CRITICAL] {replay_path.name}: Learning count {count}/{self.min_iterations} - CANNOT move/delete")
            return False
        return True
    ...
    def get_status_summary(self) -> Dict:
        ...
        completed = sum(
            1 for entry in self.status_data["replays"].values() if (
                isinstance(
                    entry,
                    dict) and entry.get(
                    "completed",
                ...
                isinstance(
                    entry,
                    int) and entry >= self.min_iterations))
