...
class IntegrationMonitor:
    ...
    def check_opponent_models(self) -> dict:
        ...
        if not self.data_dir.exists():
            logger.info("[!]  Data directory not found. No games played yet.")
    ...
    def watch_mode(self, interval: int = 60):
        ...
        logger.info("=" * 70)
        logger.info(f"Refresh interval: {interval} seconds")
        ...
        try:
            while True:
                ...
                time.sleep(interval)
                logger.info("\n" + "=" * 70)
                logger.info(f"REFRESH: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                logger.info("=" * 70)
