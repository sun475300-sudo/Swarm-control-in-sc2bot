...
def calculate_rally_point(manager):
    ...
    if manager.bot.townhalls.amount > 1 and hasattr(manager.bot, "game_info"):
        try:
            ...
            logger.debug("rally_point_calculator: %s", exc, exc_info=True)
...
def update_rally_point(manager):
    ...
    try:
        ...
        if manager.bot.townhalls.amount > 1:
            try:
                our_base = manager.bot.townhalls.closest_to(
                    manager.bot.game_info.map_center
                ).position
            except Exception:
    ...
    for unit in army_units:
        try:
            ...
            if is_idle and distance_to_rally > 5:
                ...
                # 근처에 적이 있으면 이동 명령 생략
                nearby_enemies = (
                    manager.bot.enemy_units.closer_than(12, unit)
                    if hasattr(manager.bot, "enemy_units") and manager.bot.enemy_units
                    else []
                )
