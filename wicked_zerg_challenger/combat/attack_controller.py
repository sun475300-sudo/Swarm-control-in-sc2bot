...
class AttackController:
    ...
    async def offensive_attack(self, army_units, iteration: int):
        ...
        try:
            ...
            if len(attack_targets) > 1 and army_supply >= 20:
                ...
                if iteration % 50 == 0:
                    self.logger.info(
                        f"[{int(game_time)}s] [*] MULTI-ATTACK: {num_groups} groups attacking {len(attack_targets)} targets"
                    )
            else:
                # 단일 타겟 공격
                ...
                if iteration % 50 == 0:
                    target_name = getattr(
                        attack_target, "name", str(attack_target)[:30]
                    )
                    self.logger.info(
                        f"[{int(game_time)}s] Attacking {target_name} with {army_supply} supply army ({attack_count} units ordered)"
                    )
    ...
    def find_priority_attack_target(self):
        ...
        if enemy_structures and enemy_structures.exists:
            ...
            if hasattr(self.bot, "townhalls") and self.bot.townhalls.exists:
                our_base = self.bot.townhalls.first.position
                closest_structure = min(
                    enemy_structures, key=lambda s: s.distance_to(our_base)
                )
    ...
    def get_map_search_target(self):
        ...
        if game_time - self._last_search_time > 30:
            ...
            if self.bot.iteration % 100 == 0:
                self.logger.info(
                    f"[{int(game_time)}s] Searching map location {self._search_index + 1}/{len(search_locations)}"
                )
    ...
    async def check_roach_rush_timing(self, iteration: int):
        ...
        if len(roaches) >= self._roach_rush_min_count:
            ...
            if (
                ...
                if iteration % 22 == 0:
                    self.logger.warning(
                        f"[{int(game_time)}s] {len(roaches)} ROACHES ATTACKING!"
                    )
