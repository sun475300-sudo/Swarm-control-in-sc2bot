...
class BaseDefenseSystem:
    ...
    async def execute_defense_task(self, units, threat_position):
        ...
        if enemy_units:
            for enemy in enemy_units:
                enemy_type = getattr(enemy.type_id, "name", "").upper()
                if (
                    enemy_type in self.high_priority_targets
    ...
    async def check_mandatory_base_defense(self, iteration: int) -> Optional["Point2"]:
        ...
        if max_threat_score == 0:
            if self._base_defense_active and iteration % 100 == 0:
                self.logger.info(
                    f"[{int(game_time)}s] Threat cleared - returning to normal"
                )
            self._base_defense_active = False
            self._defense_rally_point = None
            return None
        ...
        if iteration % 110 == 0:
            self.logger.warning(
                f"[{int(game_time)}s] MANDATORY DEFENSE - "
    ...
    async def execute_mandatory_defense(
        ...
        if last_stand_mode:
            if high_priority_targets:
                ...
                for unit in army_units:
                    try:
                        if unit.type_id == UnitTypeId.BANELING:
                            densest_enemy = self.find_densest_enemy_position(
                                threat_enemies
                            )
                            if densest_enemy:
                ...
                if iteration % 220 == 0:
                    self.logger.warning(
                        f"[{int(game_time)}s] LAST STAND - {len(army_units)} units - FOCUS FIRE on {getattr(main_target.type_id, 'name', 'enemy')}"
                    )
                return
    ...
    async def worker_defense(self, threat_position, threat_enemies, iteration: int):
        ...
        if last_stand_mode or defeat_level >= 3:
            ...
            if iteration % 220 == 0:
                self.logger.warning(
                    f"패배 직전! 모든 일꾼({len(defense_workers)}) 방어 참여!"
                )
        elif defeat_level >= 2:
        ...
        if iteration % 220 == 0:
            self.logger.warning(
                f"[{int(game_time)}s] {len(defense_workers)} WORKERS DEFENDING"
            )
