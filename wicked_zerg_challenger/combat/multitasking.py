    ...
    def create_task_list(
        ...
        if 60 <= game_time <= 420 and UnitTypeId:
            ...
            if 6 <= len(zerglings) <= 24:
                ...
                tasks.append(
                    ("early_harass", None, self.task_priorities["early_harass"])
                )
        ...
        if hasattr(self.bot, "enemy_structures") and game_time > 180:
            ...
            if (
                ...
                expansions = [
                    base for base in enemy_bases if base.distance_to(enemy_start) > 15
                ]

                if expansions and hasattr(self.bot, "start_location"):
                    target_expansion = min(
                        expansions, key=lambda b: b.distance_to(self.bot.start_location)
                    )
                    tasks.append(
                        (
                            "deny_expansion",
                            target_expansion.position,
                            self.task_priorities["expansion_denial"],
                        )
                    )
