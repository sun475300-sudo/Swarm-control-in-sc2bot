...
def is_too_close_to_resources(position: Point2, bot, min_distance: float = 3.0) -> bool:
    ...
    try:
        ...
        logger.debug(f"is_too_close_to_resources(module) check failed at {position}: {e}")
...
class BuildingPlacementHelper:
    ...
    def has_creep(self, position: Point2) -> bool:
        ...
        try:
            ...
            logger.debug(f"has_creep check failed at {position}: {e}")
            return False
    ...
    def is_too_close_to_resources(self, position: Point2, min_distance: float = 3.0) -> bool:
        ...
        try:
            ...
            logger.debug(f"is_too_close_to_resources check failed at {position}: {e}")
            return False
