...
def is_too_close_to_resources(position: Point2, bot, min_distance: float = 3.0) -> bool:
    ...
    try:
        ...
        logger.debug("is_too_close_to_resources(%s) errored: %s", position, exc)
...
class BuildingPlacementHelper:
    ...
    def has_creep(self, position: Point2) -> bool:
        ...
        try:
            ...
            logger.debug("BuildingPlacementHelper.has_creep(%s) errored: %s", position, exc)
            return False
    ...
    def is_too_close_to_resources(self, position: Point2, min_distance: float = 3.0) -> bool:
        ...
        try:
            ...
            logger.debug("BuildingPlacementHelper.is_too_close_to_resources(%s) errored: %s", position, exc)
            return False
