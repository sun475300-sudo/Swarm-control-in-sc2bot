...
class PersonalityModule:
    ...
    def _get_memory_greeting(self) -> Optional[str]:
        ...
        style = stats.get("dominant_style", "unknown")
        style_label = {
            "aggressive": "공격적",
            "defensive": "수비적",
            "macro": "운영형",
        ...
        style_suffix = f" {style_label} 운영을 즐기시던데," if style_label else ""

        # Format message based on win rate
        if win_rate > 70:
            return (
                f"또 오셨나요?{style_suffix} 이번엔 좀 버티시길. "
                f"(승률: {win_rate:.1f}%) gl hf."
            )
        elif win_rate < 30:
            return (
                f"지난번의 패배를 분석했습니다.{style_suffix} 이번엔 다를 겁니다. "
                f"(승률: {win_rate:.1f}%) gl hf."
            )
        else:
            return (
                f"{games+1}번째 판이네요.{style_suffix} 누가 더 발전했는지 봅시다. "
                f"(승률: {win_rate:.1f}%) gl hf."
            )
