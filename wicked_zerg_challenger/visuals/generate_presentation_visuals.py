    ...
    for i in range(n):
        ...
        fig.add_trace(
            go.Mesh3d(
                ...
                z=[0, 0, real_vals[i], real_vals[i], 0, 0, real_vals[i], real_vals[i]],
                i=[0, 0, 4, 4, 0, 2],
                j=[1, 2, 5, 6, 4, 6],
                k=[2, 3, 6, 7, 5, 7],
                color="#EF5350",
        ...
        fig.add_trace(
            go.Mesh3d(
                ...
                y=[0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8],
                z=[0, 0, sim_vals[i], sim_vals[i], 0, 0, sim_vals[i], sim_vals[i]],
                i=[0, 0, 4, 4, 0, 2],
                j=[1, 2, 5, 6, 4, 6],
                k=[2, 3, 6, 7, 5, 7],
                color="#42A5F5",
        ...
        for node in layer:
            fig.add_trace(
                go.Scatter3d(
                    ...
                    textfont=dict(size=11, color=node["color"], family="Arial Black"),
                    showlegend=False,
                )
            )
...
def ir_roi_analysis():
    ...
    # 절감률 표시 (annotation)
    cost_savings = [
        round((1 - c2 / c1) * 100) for c1, c2 in zip(cost_traditional, cost_ours, strict=False)
    ]
    time_savings = [
        round((1 - t2 / t1) * 100) for t1, t2 in zip(time_traditional, time_ours, strict=False)
    ]
...
def section2_drone_growth():
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(245,245,245,0.5)",
        legend=dict(
            font=dict(size=12), x=0.01, y=0.99, bgcolor="rgba(255,255,255,0.9)"
        ),
    ...
    fig.update_yaxes(
        title_text="드론 등록 수 (만 대)",
        secondary_y=False,
        gridcolor="rgba(200,200,200,0.5)",
    )
    fig.update_yaxes(
        title_text="성장률 (%)", secondary_y=True, showgrid=False, range=[0, 100]
    )
...
def section2_problem_heatmap():
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            zmax=10,
            text=[
                [f"{severity[i][j]}\n{annotations_text[i][j]}" for j in range(4)]
                for i in range(4)
            ],
            texttemplate="%{text}",
            textfont=dict(size=12, family="Arial Black"),
...
def section3_kpi_dashboard():
    ...
    fig.add_trace(
        go.Indicator(
            ...
            gauge=dict(
                ...
                threshold=dict(
                    line=dict(color="#C62828", width=4), thickness=0.8, value=80
                ),
            ),
            title=dict(text="목표: 80%<br>현행: 1관제사=1드론", font=dict(size=12)),
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Indicator(
            ...
            gauge=dict(
                ...
                threshold=dict(
                    line=dict(color="#C62828", width=4), thickness=0.8, value=1.0
                ),
            ),
            title=dict(text="목표: <1초<br>현행: ~15초 (수동)", font=dict(size=12)),
        ),
        row=1,
        col=2,
    )
    ...
    fig.add_trace(
        go.Indicator(
            ...
            gauge=dict(
                axis=dict(
                    range=[0, 360], ticksuffix="분", tickvals=[0, 30, 60, 120, 180, 360]
                ),
                ...
                threshold=dict(
                    line=dict(color="#C62828", width=4), thickness=0.8, value=30
                ),
            ),
            title=dict(
                text="목표: 30분 이내<br>현행: 수개월 (인프라 설치)", font=dict(size=12)
            ),
        ),
        row=2,
        col=1,
    ...
    fig.add_trace(
        go.Indicator(
            ...
            title=dict(
                text="목표: 1.0x (선형)<br>현행: 3~5x (기하급수적)", font=dict(size=12)
            ),
        ),
        row=2,
        col=2,
...
def section3_gantt_timeline():
    ...
    tasks = [
        ...
        # Phase 3: 작품 개발
        dict(
            Task="3D Boids + Authority FSM",
            Start="2026-06-01",
            Finish="2026-06-28",
            Resource="Phase 3",
        ),
        dict(
            Task="LLM(JARVIS) 지휘 시스템",
            Start="2026-06-08",
            Finish="2026-07-05",
            Resource="Phase 3",
        ),
        dict(
            Task="Redis 타이머 + FastAPI",
            Start="2026-06-08",
            Finish="2026-06-28",
            Resource="Phase 3",
        ),
...
def section3_e2e_pipeline():
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                text="Total E2E Latency: < 1초  |  전 과정 완전 자동화",
                x=0.5,
                y=-0.28,
                xref="paper",
                yref="paper",
...
def section3_sensor_fusion():
    ...
    fig.update_layout(
        ...
        annotations=[
            ...
            dict(
                text="협조 드론: 3중 센서 교차 검증  |  비협조 드론: RF + Vision 2중 탐지",
                x=0.5,
                y=-0.18,
                xref="paper",
                yref="paper",
        ...
        for node in layer["nodes"]:
            fig.add_trace(
                go.Scatter3d(
                    ...
                    text=[node["label"]],
                    textposition="top center",
                    textfont=dict(size=10, color=layer["color"], family="Arial Black"),
                    showlegend=False,
                )
            )
...
def section5_expected_effects():
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                text="모든 지표에서 Swarm-Net이 현행 대비 75~99.9% 개선",
                x=0.5,
                y=-0.22,
                xref="paper",
                yref="paper",
...
def section5_application_matrix():
    ...
    hover_text = [
        [
            f"분야: {domains[i]}<br>역량: {capabilities[j]}"
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            zmax=10,
            text=[
                [f"{scores[i][j]}\n{use_cases[i][j]}" for j in range(5)]
                for i in range(4)
            ],
            texttemplate="%{text}",
            textfont=dict(size=11, color="white", family="Arial Black"),
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="적용 분야", tickfont=dict(size=13, family="Arial Black")),
        paper_bgcolor="white",
        ...
        annotations=[
            dict(
                text="국방/방산 분야: 전 역량 최고 적합도 (평균 9.0/10)",
                x=0.5,
                y=-0.12,
                xref="paper",
                yref="paper",
...
def section5_market_tam():
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(color="#42A5F5", width=0),
            stackgroup="market",
        ),
        secondary_y=False,
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(color="#FFA726", width=0),
            stackgroup="market",
        ),
        secondary_y=False,
    )
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(250,250,250,0.5)",
        legend=dict(
            font=dict(size=11), x=0.01, y=0.99, bgcolor="rgba(255,255,255,0.9)"
        ),
...
if __name__ == "__main__":
    ...
    all_html = sorted(
        ...
        if f.endswith(".html")
        and (
            f.startswith("capstone_") or f.startswith("ir_") or f.startswith("section")
        )
    )
