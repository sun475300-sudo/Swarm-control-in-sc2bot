...
def tb(slide, lt, t, w, h, txt, sz=18, c=WHITE, b=False, al=PP_ALIGN.LEFT, fn=FONT):
    box = slide.shapes.add_textbox(lt, t, w, h)
    tf = box.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
    p.text = txt
...
def bullets(slide, lt, t, w, h, items, sz=14, c=LIGHT):
    box = slide.shapes.add_textbox(lt, t, w, h)
    tf = box.text_frame
    tf.word_wrap = True
...
def card(slide, lt, t, w, h, border=None):
    s = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, lt, t, w, h)
    s.fill.solid()
...
def tag(slide, lt, t, txt, bg_c=CYAN, txt_c=BG):
    s = slide.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, lt, t, Inches(3.2), Inches(0.34)
    )
    s.fill.solid()
...
def accent_line(slide, lt, t):
    s = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, lt, t, Inches(0.8), Pt(3))
    s.fill.solid()
...
def footer_text(slide, txt):
    tb(slide, Inches(0.8), Inches(6.55), Inches(11.5), Inches(0.3), txt, sz=9, c=DIM)
...
def slide_01(prs):
    ...
    for i, (ix, iy, isz, rot) in enumerate(
        ...
        h = s.shapes.add_shape(
            MSO_SHAPE.HEXAGON, Inches(ix), Inches(iy), Inches(isz), Inches(isz)
        )
        h.fill.background()
...
def slide_03(prs):
    ...
    tb(
        ...
        Inches(11),
        Inches(0.35),
        "어떤 도시도 규모별 이동식 레이더 망을 배치한 적 없음",
        sz=13,
        c=LIGHT,
    )
...
def slide_04(prs):
    ...
    tb(
        ...
        Inches(5.2),
        Inches(0.6),
        "복잡한 다중 에이전트 시스템을 검증하는 가장 빠른 방법",
        sz=13,
        c=LIGHT,
    )
...
def slide_12(prs):
    ...
    for i, (num, title, amount, desc, ac) in enumerate(revenues):
        ...
        circ = s.shapes.add_shape(
            MSO_SHAPE.OVAL, Inches(1.0), y + Inches(0.1), Inches(0.45), Inches(0.45)
        )
        circ.fill.solid()
        circ.fill.fore_color.rgb = ac
...
def slide_15(prs):
    ...
    tbl_s = s.shapes.add_table(
        rows, cols, Inches(0.6), Inches(2.0), Inches(6.0), Inches(2.2)
    )
    tbl = tbl_s.table
...
def slide_18(prs):
    ...
    footer_text(
        s, "마일스톤: SC2 Top 5 (2027) | 1K 드론 시뮬 (2028) | 100대 테스트 (2030)"
    )
    footer(s, 18)
...
def slide_19(prs):
    ...
    tb(
        ...
        Inches(11),
        Inches(0.4),
        "엑시트 전략: IPO (2035) 또는 방산/항공 대기업 인수",
        sz=14,
        c=PURPLE,
        b=True,
    )
