...
def radar_system_overview():
    ...
    user_colors = ["#2196F3", "#2196F3", "#2196F3", "#FF9800", "#2196F3", "#4CAF50"]
...
def radar_mesh_network():
    ...
    fig.update_layout(
        title=dict(
            text="군집 드론 레이더 망 구조 (Mesh Network)",
...
def radar_operation_concept():
    ...
    for u in users:
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers+text",
                marker=dict(
                    size=10, color=u["color"], line=dict(width=2, color="white")
                ),
                text=[u["short"]],
                textposition="top center",
    ...
    fig.update_layout(
        title=dict(
            text="군집 드론 레이더 관제: 전체 운용 개념도",
            font=dict(size=20, family="Arial Black"),
            x=0.5,
        ),
        scene=dict(
            xaxis=dict(title="X (m)", range=[-13, 13], gridcolor="#f0f0f0"),
            yaxis=dict(title="Y (m)", range=[-13, 13], gridcolor="#f0f0f0"),
            zaxis=dict(title="고도 (m)", range=[-5, 120], gridcolor="#f0f0f0"),
