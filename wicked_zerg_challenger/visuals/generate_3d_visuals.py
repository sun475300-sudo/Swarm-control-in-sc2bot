...
def part1_fsm_3d():
    ...
    # 메인 타임라인 (3D 파이프)
    for _i, (phase, t, col) in enumerate(zip(phases, times, colors)):
        # 메인 노드 (큰 구)
        fig.add_trace(
            go.Scatter3d(
                x=[t],
                ...
                mode="markers+text",
                marker=dict(
                    size=20, color=col, opacity=0.9, line=dict(width=2, color="white")
                ),
                text=[phase],
    ...
    fig.update_layout(
        title=dict(text="SC2 Bot: Game Phase FSM (3D Timeline)", font=dict(size=20)),
        scene=dict(
...
def part1_authority_3d():
    ...
    fig.update_layout(
        title=dict(
            text="Authority Mode: Dynamic Priority Switching (3D)", font=dict(size=20)
        ),
        scene=dict(
...
def part1_hybrid_3d():
    ...
    fig.update_layout(
        title=dict(
            text="Rule-Based + RL Hybrid Architecture (3D Layers)", font=dict(size=20)
        ),
        scene=dict(
            xaxis=dict(title="", showticklabels=False, showgrid=False),
            yaxis=dict(title="", showticklabels=False, showgrid=False),
...
def part2_tactical_3d():
    ...
    fig.update_layout(
        title=dict(text="Tactical Decision Chain (3D Spiral)", font=dict(size=20)),
        scene=dict(
            xaxis=dict(title="", showticklabels=False, showgrid=False),
            yaxis=dict(title="", showticklabels=False, showgrid=False),
...
def part2_engagement_3d():
    ...
    fig.update_layout(
        title=dict(text="Engagement Decision Tree (3D)", font=dict(size=20)),
        scene=dict(
            xaxis=dict(title="", showticklabels=False, showgrid=False),
            yaxis=dict(title="", showticklabels=False, showgrid=False),
...
def part3_boids_3d():
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            marker=dict(
                size=[
                    6 if t == "Zergling" else 8 if t == "Roach" else 7
                    for t in unit_types
                ],
                color=unit_colors,
                opacity=0.85,
    ...
    # 플레이/슬라이더
    fig.update_layout(
        title=dict(text="Boids Swarm Attack Simulation (3D)", font=dict(size=20)),
        scene=dict(
                ...
                steps=[
                    dict(
                        args=[
                            [f"frame_{i}"],
                            dict(
                                mode="immediate", frame=dict(duration=300, redraw=True)
                            ),
                        ],
                        method="animate",
                        label=f"T{i}",
...
def part3_forces_3d():
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            mode="markers+text",
            marker=dict(
                size=15, color="#4CAF50", opacity=1, line=dict(width=2, color="white")
            ),
            text=["ME"],
            textposition="top center",
    ...
    fig.update_layout(
        title=dict(text="Boids Force Vectors (3D Visualization)", font=dict(size=20)),
        scene=dict(
...
def part4_concept_mapping_3d():
    ...
    for i, (name, col) in enumerate(sc2_modules):
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers+text",
                marker=dict(
                    size=12, color=col, opacity=0.9, line=dict(width=1, color="white")
                ),
                text=[name],
    ...
    fig.update_layout(
        title=dict(
            text="SC2 → Drone ATC: Technology Transfer Bridge (3D)", font=dict(size=20)
        ),
        scene=dict(
...
def part4_drone_atc_3d():
    ...
    for alt, name, col, n_drones in altitudes:
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                z=drone_z,
                mode="markers",
                marker=dict(size=5, color=col[:7], opacity=0.8, symbol="diamond"),
                showlegend=False,
    ...
    fig.update_layout(
        title=dict(text="Drone ATC: 3D Airspace Management", font=dict(size=20)),
        scene=dict(
...
def part4_roadmap_3d():
    ...
    for name, x, col, details in stages:
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                mode="markers+text",
                marker=dict(
                    size=22, color=col, opacity=0.9, line=dict(width=2, color="white")
                ),
                text=[name],
                textposition="top center",
                textfont=dict(size=12, color=col, family="Arial Black"),
    ...
    fig.update_layout(
        title=dict(
            text="End-to-End Vision: Game → Reality (3D Roadmap)", font=dict(size=20)
        ),
        scene=dict(
