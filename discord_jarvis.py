...
try:
    ...
    logging.warning(f"CommandDispatcher/keyword_handlers 임포트 실패: {e}")
try:
...
try:
    ...
    SEARCH_RESULT_LIMIT = 1800
    ATTACHMENT_TEXT_LIMIT = 15000
    PROMPT_TRUNCATE_LIMIT = 8000
...
class JarvisBot(commands.Bot):
    ...
    async def _try_local_response(self, message: discord.Message, prompt: str) -> bool:
        ...
        # ══════════════════════════════════════════
        # ── 관리자 전용 / self 참조 필요 핸들러 (keyword_handlers 이관 불가) ──
        # ══════════════════════════════════════════
    ...
    async def _execute_tool(self, name, args, message, user_id):
        """도구명과 인자를 받아 해당 도구 실행. ToolExecutor로 위임."""
        if self._tool_executor:
            return await self._tool_executor.execute(name, args, message, user_id)
        return f"Tool Error: ToolExecutor 모듈을 불러올 수 없습니다."
