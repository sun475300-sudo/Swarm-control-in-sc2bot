...
def load_replay_logs(log_dir: str = "data/replays") -> dd.DataFrame:
    ...
    mock_data = pd.DataFrame({
        "game_id":       range(1000),
        "map":           np.random.choice(["Equilibrium LE", "Site Delta LE", "Gresvan LE"], 1000),
...
def compute_apm_features(ddf: dd.DataFrame) -> da.Array:
    ...
    arr = da.from_array(np.column_stack([
        apm_vals,
        np.log1p(apm_vals),
...
def build_stats_graph(ddf: dd.DataFrame) -> dict:
    ...
    apm_buckets = ddf["apm"].map_partitions(
        lambda s: pd.cut(s, bins=[0, 100, 150, 200, 250, 999], labels=["<100","100-150","150-200","200-250","250+"])
    )
...
def execute_graph(graph: dict, client: Optional[Client] = None) -> dict:
    ...
    if client:
        ...
        results = {k: v.compute() if hasattr(v, "compute") else v
