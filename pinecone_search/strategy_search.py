...
class StrategyEmbedding:
    ...
    def encode_game_state(self, gs: GameState) -> List[float]:
        ...
        v[7] = self._norm(
            gs.mineral_income * 60, self.MAX_INCOME_MIN
        )  # per-second to per-min
        ...
        v[19] = self._norm(
            gs.army_value - gs.enemy_army_value + 100, 200.0
        )  # army advantage
        ...
        v[53] = (
            1.0 if gs.enemy_army_value < gs.army_value * 0.5 else 0.0
        )  # dominating flag
        ...
        v[61] = (
            float(gs.army_supply) / max(gs.worker_count, 1) / 5.0
        )  # aggression index
        v[62] = float(gs.base_count * 400 - gs.minerals) / 2000.0  # saturation deficit
        v[63] = (
            float(gs.tech_buildings + gs.production_buildings)
            / max(gs.supply_cap, 1)
            * 10
        )
    ...
    def encode_strategy(self, strategy: Strategy) -> List[float]:
        ...
        gs = GameState(
            ...
            phase=(
                GamePhase.EARLY
                if timing < 240
                else GamePhase.MID if timing < 720 else GamePhase.LATE
            ),
            enemy_base_count=1,
            enemy_tech_tier=1,
        )
...
class StrategyIndex:
    ...
    def __init__(
        ...
        if _PINECONE_AVAILABLE and api_key:
            try:
                ...
                logger.warning(
                    "Pinecone init failed (%s) — using in-memory fallback.", exc
                )
                self._index = None
    ...
    def query_by_game_state(
        ...
        if self._index is not None:
            ...
            if filter_race is not None:
                matches = [
                    m for m in matches if m["metadata"].get("race") == filter_race.value
                ]
            if min_win_rate > 0.0:
                matches = [
                    m
        ...
        for rank, match in enumerate(matches, start=1):
            ...
            if sid in self._strategy_cache:
                ...
                strat = Strategy(
                    ...
                    typical_timing_seconds=float(
                        meta.get("typical_timing_seconds", 300)
                    ),
...
if __name__ == "__main__":
    ...
    query_scenarios = [
        ...
        },
        {
            ...
            "state": GameState(
                minerals=300,
                vespene=150,
                mineral_income=900.0,
                vespene_income=250.0,
                worker_count=26,
                supply_used=80,
                supply_cap=90,
                army_supply=50,
                base_count=2,
                tech_buildings=3,
                production_buildings=4,
                army_value=50,
                enemy_army_value=45,
                game_time_seconds=390,
        ...
        },
        {
            ...
            "state": GameState(
                minerals=600,
                vespene=400,
                mineral_income=1400.0,
                vespene_income=600.0,
                worker_count=66,
                supply_used=160,
                supply_cap=180,
                army_supply=90,
    ...
    for scenario in query_scenarios:
        ...
        for r in results:
            print(
                f"  #{r.rank}  [{r.score:.4f}]  {r.strategy.name!r:<35} "
