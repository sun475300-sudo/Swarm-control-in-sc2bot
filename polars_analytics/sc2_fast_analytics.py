...
def build_lazy_pipeline(path: str) -> pl.LazyFrame:
    ...
    return (
        ...
        .with_columns([
            (pl.col("minerals_spent") + pl.col("gas_spent")).alias("total_resources"),
            pl.col("apm").cast(pl.Float64),
            pl.when(pl.col("winner")).then(pl.lit(1)).otherwise(pl.lit(0)).alias("win_int"),
        ])
        .with_columns([
...
def process_replay_batch(replay_files: list[str]) -> pl.DataFrame:
    ...
    return (
        combined
        .filter(pl.col("apm") > 30)
        .with_columns([
            (pl.col("minerals_spent") / pl.col("game_duration")).alias("minerals_per_min"),
            (pl.col("workers_produced") / pl.col("game_duration")).alias("workers_per_min"),
        ])
...
def run_sc2_analytics(data_dir: str = "data/replays") -> None:
    ...
    tier_counts = tiered.group_by("skill_tier").agg(pl.col("game_id").count().alias("count"))
