...
def create_train_state(
    rng: jax.random.KeyArray, lr: float = 1e-3
...
@jax.jit
def pg_loss(
    params: dict, states: jnp.ndarray, actions: jnp.ndarray, returns: jnp.ndarray
) -> jnp.ndarray:
...
# ---------------------------------------------------------------------------
def train(
    num_episodes: int = 200, lr: float = 1e-3, seed: int = 42
) -> train_state.TrainState:
    ...
    for ep in range(1, num_episodes + 1):
        ...
        ts, loss = update_step(
            ts, episode["states"], episode["actions"], episode["returns"]
        )
        ...
        if ep % 50 == 0:
            ...
            print(
                f"  Episode {ep:4d}  avg_loss={avg_loss:.4f}  "
