...
def generate_production_data(n: int = 500, shift: bool = True) -> pd.DataFrame:
    ...
    np.random.seed(99)
    df = pd.DataFrame({
        "apm": np.random.normal(140 if shift else 120, 35, n).clip(40, 300),  # Shift: higher APM
        "supply_used": np.random.normal(155 if shift else 140, 20, n).clip(50, 200),
        "minerals_spent": np.random.normal(14000 if shift else 12000, 5000, n).clip(2000, 30000),
        "gas_spent": np.random.normal(6000 if shift else 5000, 2500, n).clip(500, 15000),
        "game_duration": np.random.normal(420 if shift else 450, 160, n).clip(90, 1800),
        "army_value": np.random.normal(9500 if shift else 8000, 3500, n).clip(500, 20000),
        "workers_produced": np.random.normal(50 if shift else 45, 13, n).clip(10, 80),
        "race_enc": np.random.randint(0, 3, n),
        "winner": np.random.randint(0, 2, n),
        "predicted_winner": np.random.randint(0, 2, n),
    })
...
def build_game_state_drift_report(reference: pd.DataFrame, current: pd.DataFrame) -> Report:
...
def build_model_performance_report(reference: pd.DataFrame, current: pd.DataFrame) -> Report:
...
def build_monitoring_test_suite(reference: pd.DataFrame, current: pd.DataFrame) -> TestSuite:
