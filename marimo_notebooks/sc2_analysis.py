...
def match_table(filtered):
    ...
    display_df["duration"] = display_df["duration"].apply(lambda s: f"{s//60}:{s%60:02d}")

    table = mo.ui.table(
        data=display_df[["date", "map", "race", "result", "duration", "apm", "mmr"]].to_dict("records"),
...
def mmr_chart(filtered):
    ...
    chart_df.columns = list(chart_df.columns[:-1]) + ["mmr"] if "mmr" in chart_df.columns else chart_df.columns
    chart_df["game_num"] = range(len(chart_df))

    chart = alt.Chart(chart_df).mark_line(
        color="#4c72b0", strokeWidth=2
    ).encode(
...
def win_rate_chart(filtered):
    ...
    chart = alt.Chart(wr_df).mark_bar().encode(
...
def apm_chart(filtered):
    ...
    chart = alt.Chart(filtered).mark_bar(opacity=0.7).encode(
...
@app.cell
def layout(title_cell, slider, race_dropdown, map_select,
            summary_stats, match_table, mmr_chart,
            win_rate_chart, apm_chart):
    return mo.vstack([
        title_cell,
