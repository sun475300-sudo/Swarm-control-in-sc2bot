...
def compute_derived_features(inputs: pd.DataFrame) -> pd.DataFrame:
    ...
    df = pd.DataFrame()
    df["supply_ratio"] = (
        inputs["supply_used"] / inputs["supply_cap"].clip(lower=1)
    ).astype("float32")
    df["resource_efficiency"] = (
        inputs["army_value"] / (inputs["minerals_banked"] + inputs["gas_banked"] + 1)
    ).astype("float32")
    df["is_supply_blocked"] = (inputs["supply_used"] >= inputs["supply_cap"]).astype(
        int
    )
    return df
    ...
    pd.DataFrame(
        {
            ...
            "preferred_opening": np.random.choice(
                ["Pool First", "Hatch First", "Gas First"], n
            ),
        }
...
if __name__ == "__main__":
    ...
    print(
        f"  Feature Views: unit_features, game_context_features, player_history_features"
    )
