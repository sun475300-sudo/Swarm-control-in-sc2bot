...
def add_bullet_list(slide, left, top, width, height, items, font_size=16, color=LIGHT):
    txBox = slide.shapes.add_textbox(left, top, width, height)
    tf = txBox.text_frame
    tf.word_wrap = True
...
def add_rounded_rect(
    slide, left, top, width, height, fill_color=BG_CARD, border_color=None
):
...
def add_tag_label(slide, left, top, text, bg_color=CYAN, text_color=BG_DARK, w=None):
...
def add_footer(slide, slide_num):
    ...
    add_text_box(
        ...
        Inches(2),
        Inches(0.4),
        f"{slide_num:02d} / {TOTAL_SLIDES:02d}",
        font_size=11,
        color=DIM,
        font_name=FONT_CODE,
...
def build_slide_01(prs):
    ...
    add_tag_label(
        slide, Inches(0.8), Inches(1.2), "CAPSTONE PROJECT 2026", CYAN, BG_DARK
    )
    ...
    add_image(
        slide,
        V2_IMG_DIR / "slide1_swarm_hex.png",
        Inches(8.3),
        Inches(1.0),
        width=Inches(4.5),
    )
...
def build_slide_02(prs):
    ...
    add_image(
        slide,
        V2_IMG_DIR / "slide2_radar_gap.png",
        Inches(7.5),
        Inches(0.3),
        width=Inches(5.5),
    )
    ...
    add_tag_label(
        slide, Inches(0.8), Inches(0.6), "BACKGROUND & PROBLEM", ORANGE, BG_DARK
    )
...
def build_slide_03(prs):
    ...
    add_image(
        slide,
        V2_IMG_DIR / "slide3_mobile_atc.png",
        Inches(8.8),
        Inches(0.8),
        width=Inches(4.0),
    )
    ...
    pillars = [
        (
            "Swarm Fleet",
            "6~12대 UAV가 정다각형 대형을\n자율 형성, 3D 다층 레이더 돔 전개",
            CYAN,
        ),
        (
            "Mesh Radar",
            "LiDAR + RF 기반 Mesh Network\n삼각측량으로 진입 객체 3D 좌표 산출",
            GREEN,
        ),
        (
            "Timer Control",
            "비인가 UAV에 체공 시간 자동 할당\n만료 시 RTH 명령 및 에스코트 프로토콜",
            ORANGE,
        ),
    ]
...
def build_slide_04(prs):
    ...
    add_tag_label(
        slide, Inches(0.8), Inches(0.5), "CORE TECHNOLOGY: SIM-TO-REAL", PURPLE, BG_DARK
    )
    ...
    table_data = [
        ...
        (
            "IntelManager (정보 수집/탐지)",
            "3D Radar Mesh (Sensor Fusion)",
            "적응형 파라미터 튜닝",
        ),
        (
            "RL Agent (강화학습 의사결정)",
    ...
    tbl_shape = slide.shapes.add_table(
        rows, cols, Inches(0.6), Inches(4.5), Inches(11.5), Inches(2.4)
    )
    tbl = tbl_shape.table
...
def build_slide_05(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "SYSTEM ARCHITECTURE", CYAN_BLUE, BG_DARK
    )

    # UTM slide PNG as main visual (left side)
    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_03.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
...
def build_slide_06(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "BOIDS 3D SWARM CONTROL", GREEN, BG_DARK
    )

    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_04.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
    ...
    forces = [
        ("Separation", "w=2.0", RED),
        ("Alignment", "w=1.0", CYAN_BLUE),
        ("Cohesion", "w=1.0", GREEN),
        ("Target", "w=1.5", GOLD),
        ("Obstacle", "w=2.5", PURPLE),
        ("Alt Hold", "w=1.0", RGBColor(0x38, 0xBD, 0xF8)),
        ("Terrain", "w=3.0", ORANGE),
        ("Corridor", "동적", RGBColor(0xE8, 0x79, 0xF9)),
    ]
    for i, (name, weight, clr) in enumerate(forces):
        y = Inches(2.1 + i * 0.52)
        dot = slide.shapes.add_shape(
            MSO_SHAPE.OVAL, Inches(8.35), y + Inches(0.08), Inches(0.15), Inches(0.15)
        )
        dot.fill.solid()
...
def build_slide_07(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "TTC COLLISION PREDICTION", RED, BG_DARK
    )

    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_05.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
...
def build_slide_08(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "FLIGHT CORRIDOR SYSTEM", CYAN_BLUE, BG_DARK
    )

    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_06.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
    ...
    for i, (name, desc, clr) in enumerate(layers):
        y = Inches(1.85 + i * 0.6)
        add_rounded_rect(slide, Inches(8.3), y, Inches(4.5), Inches(0.5), BG_CARD, clr)
        add_text_box(
            slide,
            Inches(8.5),
            y + Inches(0.05),
    ...
    for i, (pct, desc, clr) in enumerate(profiles):
        x = Inches(8.3 + i * 1.5)
        add_tag_label(
            slide, x, Inches(5.2), f"{pct} {desc}", clr, BG_DARK, w=Inches(1.4)
        )
...
def build_slide_09(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "AUTHORITY MODE FSM", PURPLE, BG_DARK
    )

    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_07.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
...
def build_slide_10(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "DATA MODEL & DRONE STATE", GOLD, BG_DARK
    )

    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_08.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
...
def build_slide_11(prs):
    ...
    add_tag_label(
        slide, Inches(0.3), Inches(0.3), "FAIL-SAFE & SELF HEALING", RED, BG_DARK
    )

    add_image(
        slide,
        UTM_IMG_DIR / "utm_slide_09.png",
        Inches(0.2),
        Inches(0.8),
        width=Inches(7.8),
    )
...
def build_slide_12(prs):
    ...
    add_image(
        slide,
        V2_IMG_DIR / "slide5_3d_tracking.png",
        Inches(6.5),
        Inches(0.3),
        width=Inches(6.5),
    )
...
def build_slide_13(prs):
    ...
    add_image(
        slide,
        V2_IMG_DIR / "slide6_timer_dash.png",
        Inches(7.0),
        Inches(0.3),
        width=Inches(6.0),
    )
    ...
    add_tag_label(
        slide, Inches(0.8), Inches(0.6), "REAL-TIME CONTROL DASHBOARD", ORANGE, BG_DARK
    )
...
def build_slide_14(prs):
    ...
    add_image(
        slide,
        V2_IMG_DIR / "slide7_comparison.png",
        Inches(6.5),
        Inches(2.8),
        width=Inches(6.5),
    )
    ...
    add_tag_label(
        slide, Inches(0.8), Inches(0.5), "EXPECTED IMPACT & APPLICATIONS", CYAN, BG_DARK
    )
    ...
    rows, cols = len(table_data), 3
    tbl_shape = slide.shapes.add_table(
        rows, cols, Inches(0.6), Inches(2.4), Inches(6.2), Inches(3.2)
    )
    tbl = tbl_shape.table
...
def build_slide_15(prs):
    ...
    add_tag_label(
        slide, Inches(4.5), Inches(0.5), "CONCLUSION & ROADMAP", CYAN, BG_DARK
    )
    ...
    # Completed (left)
    add_rounded_rect(
        slide, Inches(0.3), Inches(2.6), Inches(4.5), Inches(2.3), BG_CARD, CYAN
    )
    add_text_box(
        slide,
        Inches(0.5),
    ...
    add_bullet_list(
        ...
        Inches(4.1),
        Inches(1.8),
        [(c, DIM, False) for c in completed],
        font_size=12,
    )
