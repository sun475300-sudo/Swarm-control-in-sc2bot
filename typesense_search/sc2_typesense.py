    ...
    "fields": [
        ...
        # Semantic vector field (1536-dim for OpenAI, 384-dim for sentence-transformers)
        {
            ...
            "embed": {
                ...
                "model_config": {"model_name": "ts/all-MiniLM-L6-v2"},
            },
        },
    ],
...
def add_builds(builds: List[dict]):
    resp = client.collections["sc2_builds"].documents.import_(
        builds, {"action": "upsert"}
    )
...
if __name__ == "__main__":
    ...
    for hit in results["hits"]:
        print(
            f"  - {hit['document']['build_name']} score={hit.get('text_match_info', {}).get('score', 'N/A')}"
        )
