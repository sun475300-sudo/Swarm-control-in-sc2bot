...
|---|-----|----------|----------------------|---------------------|------|--------|
| B1 | **HIGH** | `wicked_zerg_challenger/opponent_modeling.py:341 vs 765` | The 341-line version is the **comprehensive** opponent-modeling step: build-order tracking, timing-attack detection, tech-progression tracking, blackboard updates. | The 765-version is a **stub** that only calls `_detect_early_signals`. **Whole opponent-modeling feature is silently disabled** by the duplicate. | Move comprehensive logic from 341 into the active method (765), remove the duplicate. | Pending |
| B2 | High | `wicked_zerg_challenger/economy_manager.py:1298 vs 2507` (`_prevent_resource_banking`) | First has tighter early-game defense banking via `EconomyConfig.BANKING_*` thresholds. | Second uses blackboard-aware threat checks, but lacks the config thresholds. | Merge — keep blackboard threat logic, also keep config thresholds. Remove duplicate. | Pending |
