...
def apm_folder(state: APMState, event: GameEvent) -> APMState:
    ...
    return APMState(
        ...
        window_start=state.window_start or event.timestamp,
    )
...
def compute_apm(metadata: tuple, state: APMState) -> dict:
    ...
    duration_minutes = (
        window_meta.close_time - window_meta.open_time
    ).total_seconds() / 60
    apm = state.action_count / duration_minutes if duration_minutes > 0 else 0
    return {
        "player_id": metadata[0] if isinstance(metadata, tuple) else "unknown",
        "apm": round(apm, 1),
        "window_actions": state.action_count,
    }
...
def build_dataflow() -> Dataflow:
    ...
    sample_events = [
        ...
        },
        {
            ...
            "player_id": "ZergBot",
            "event_type": "game_end",
            "timestamp": "2026-01-01T10:07:00+00:00",
            "value": 0.0,
            "metadata": {"result": "win"},
        },
        {
        ...
        },
        {
            ...
            "player_id": "ZergBot",
            "event_type": "game_end",
            "timestamp": "2026-01-01T11:06:00+00:00",
            "value": 0.0,
