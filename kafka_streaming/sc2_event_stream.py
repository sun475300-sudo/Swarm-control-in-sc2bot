...
class SC2EventProducer:
    ...
    def send_unit_created(self, game_id: int, game_loop: int, unit_type: str, team: str, x: float, y: float):
        event = UnitCreatedEvent(
            game_id=game_id, game_loop=game_loop,
            unit_type=unit_type, team=team, x=x, y=y,
            timestamp=datetime.utcnow().isoformat(),
        )
        self.send_event(TOPIC_GAME_EVENTS, asdict(event), key=str(game_id))

    def send_unit_died(self, game_id: int, game_loop: int, unit_type: str, team: str, killer: str):
        event = UnitDiedEvent(
            game_id=game_id, game_loop=game_loop,
            unit_type=unit_type, team=team, killer=killer,
            timestamp=datetime.utcnow().isoformat(),
        )
        self.send_event(TOPIC_GAME_EVENTS, asdict(event), key=str(game_id))

    def send_resource_change(self, game_id: int, game_loop: int, minerals: int, vespene: int,
    ...
    def send_action(self, game_id: int, game_loop: int, action: str, x: float, y: float):
        event = ActionTakenEvent(
            game_id=game_id, game_loop=game_loop,
            action=action, target_x=x, target_y=y,
            timestamp=datetime.utcnow().isoformat(),
        )
        self.send_event(TOPIC_GAME_EVENTS, asdict(event), key=str(game_id))
...
class SC2AnalyticsConsumer:
    def __init__(self, group_id: str = "sc2-analytics", config: dict = None):
        cfg = {**(config or KAFKA_CONFIG), "group.id": group_id, "auto.offset.reset": "earliest"}
        self.consumer = Consumer(cfg)
        self.running  = False
        self.stats    = {"unit_created": 0, "unit_died": 0, "resource_change": 0, "action_taken": 0}
...
def simulate_game_events(producer: SC2EventProducer, game_id: int = 1):
    ...
    for loop in range(0, 500, 50):
        producer.send_resource_change(game_id, loop, minerals=400+loop*2, vespene=200+loop,
                                       supply_used=12+loop//50, supply_cap=22)
        time.sleep(0.05)

    for i in range(5):
        producer.send_unit_created(game_id, 100+i*10, "Zergling", "player", 20.0+i, 30.0)
