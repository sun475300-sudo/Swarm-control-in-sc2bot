...
# ---------------------------------------------------------------------------
spark = (
    SparkSession.builder.appName("SC2_BattleLogProcessor")
...
df = spark.read.option("header", "true").schema(battle_schema).csv(INPUT_PATH)

# ---------------------------------------------------------------------------
# Parse & clean structured data
# ---------------------------------------------------------------------------
df_clean = (
    df.filter(F.col("matchup").isin("ZvT", "ZvZ", "ZvP"))
    .withColumn("is_win", (F.col("result") == "win").cast(BooleanType()))
    .withColumn(
        "kill_death_ratio",
        F.when(
            F.col("units_lost") > 0, F.col("units_killed") / F.col("units_lost")
        ).otherwise(F.col("units_killed").cast(FloatType())),
    )
    .withColumn(
        "resource_efficiency",
        F.when(
            (F.col("minerals_used") + F.col("gas_used")) > 0,
...
top_builds = (
    df_clean.groupBy("matchup", "build_order")
    .agg(
        F.count("*").alias("games"),
        F.round(F.mean(F.col("is_win").cast("int")) * 100, 2).alias("win_rate_pct"),
