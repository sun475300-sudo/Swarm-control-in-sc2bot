...
## 0. 2026-07-18 상태 업데이트 (읽고 시작할 것 — 아래 §1 수치는 4월 기준으로 낡음)

이 문서가 처음 작성된 4/26 시점엔 열린 PR 16건이었으나, 2026-07-18 기준
**열린 PR 487건 / 머지 8건**(마지막 머지: PR #218, 2026-06-01)으로 폭증했다.
원인 분석은 신규 파일 `CLAUDE.md`에 상세 기록 — 향후 자동화 세션은 작업 시작 전
그 파일을 먼저 읽을 것. 핵심 요약:

- 새 세션마다 이전 세션들의 열린 PR을 모르는 상태로 시작 → 동일한 두 근본 원인
  (`test_combat_phase_fsm.py`의 `asyncio.get_event_loop()` deprecated 패턴,
  `sc2bot-ci.yml` lint 잡의 repo-wide black/isort 포맷 검사)을 계속 재발견하고
  매번 새 draft PR을 열었다 (#495–#525 근처가 거의 전부 동일 수정의 중복).
- **머지는 여전히 사용자 전용** — 자동화는 머지도, 대량 close도 하지 않는다
  (본 문서 §3 정책 그대로 유효).
- 권장 조치(사용자 판단 필요): PR #521("FSM/sys.path 버그 + repo-wide black/isort,
  strict lint gate를 통과하는 첫 PR")을 머지 후보로 검토 → 머지되면 나머지
  #471–#525 범위 대부분을 중복으로 close 가능.
- 예약 작업(cron) 실행 간격이 사용자가 요청한 "매일"이 아니라 **거의 매시간**으로
  관측됨 — 이것이 폭증의 실질 원인이므로 스케줄 설정 재검토 권장.
