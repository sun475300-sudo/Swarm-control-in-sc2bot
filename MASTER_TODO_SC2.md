...
## 0. 2026-07-19 상태 업데이트 (읽고 시작할 것 — 아래 §1 수치는 4월 기준으로 낡음)

이 문서 작성 시점(4/26)엔 열린 PR 16건이었으나, 2026-07-19 기준
**열린 PR 약 504건 / 머지 2건**(#218, #533)으로 폭증했다. 자동화 세션은
작업 시작 전 신규 파일 `CLAUDE.md`를 먼저 읽을 것 — 원인 분석과 행동 수칙이
그 파일에 있다. 핵심 요약:

- `tests/test_combat_phase_fsm.py`의 `asyncio.get_event_loop()` deprecated
  버그를 놓고 동일 시각대(2026-07-18)에 13개 가까운 중복 PR이 열렸다. **이
  버그는 이미 main에 수정 반영되어 있음**(2026-07-19 확인: 23/23 pass) —
  해당 PR들은 전부 폐기 대상.
- **머지는 여전히 사용자 전용** — 자동화는 머지도, 대량 close도 하지 않는다
  (본 문서 §3 정책 그대로 유효).
- `tests/` 스위트는 (실제 `sc2` 패키지가 필요한 파일 1개 제외) 395 passed /
  33 skipped로 정상. `wicked_zerg_challenger/tests/`는 샌드박스에 따라
  `sc2`(burnysc2) 설치가 실패할 수 있음(mpyq 빌드 오류) — 이는 환경 문제이지
  코드 버그가 아님.
- 예약 작업(cron) 실행 간격이 요청된 "매일"이 아니라 훨씬 잦은 주기로
  관측됨 — 이것이 폭증의 실질 원인. 스케줄/트리거 설정 재검토 필요.
