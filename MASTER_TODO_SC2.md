...
## 0. 긴급 — PR 적체 위기 (2026-07-16 갱신, 사용자 확인 필요)

- **열린 draft PR 440개 이상** (`#15`–`#485`, 2026-04-20 ~ 2026-07-16). **`#218` 단 1건만 병합됨.**
- 반복되는 "test → fix → commit → push" 세션이 매번 동일한 `asyncio.get_event_loop()`
  FSM 테스트 버그를 독립적으로 재발견·재수정하고 있음 — 근본 원인은 CI의
  `pytest tests/ -v --tb=short --co -q` (`--co` = collect-only)가 **실제로 테스트를 실행한 적이
  없어서** 회귀가 계속 통과된 것처럼 보였기 때문. 이번 세션에서 `.github/workflows/ci.yml`을
  수정해 실제로 테스트가 실행되도록 고쳤음(아래 참고).
- **권장 조치 (자동 실행 안 함 — 사용자 결정 필요)**:
  1. 가장 최신·완성도 높은 후보(`#481`/`#485` 계열)를 검토해 하나만 `main`에 병합
  2. 나머지 ~440개 중복 draft PR은 일괄 close
  3. "모든 PR은 사용자 검토 후 병합" 정책이 적체의 실제 병목이므로, CI-green PR 자동 병합
     허용 여부 재검토
- 상세 내용은 PR `#482` ("docs: PR backlog audit")에 이미 정리되어 있음.
