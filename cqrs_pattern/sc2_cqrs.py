...
class WriteModel:
    ...
    def _handle_build_unit(self, cmd: Command) -> CommandResult:
        ...
        return CommandResult(
            success=True, events=[event], correlation_id=cmd.correlation_id
        )
    ...
    def _handle_expand(self, cmd: Command) -> CommandResult:
        ...
        event = DomainEvent(
            ...
            payload={
                "location": location,
                "expansion_count": self._game_state["expansions"],
            },
        )
        return CommandResult(
            success=True, events=[event], correlation_id=cmd.correlation_id
    ...
    def _handle_research_upgrade(self, cmd: Command) -> CommandResult:
        ...
        if (
            self._game_state["minerals"] < cost_minerals
            or self._game_state["vespene"] < cost_vespene
        ):
            return CommandResult(
                success=False,
...
class WinRateProjection(ReadModelProjection):
    ...
    def __init__(self) -> None:
        ...
        self._losses = 0
        self._draws = 0
        self._race_stats: Dict[str, Dict[str, int]] = defaultdict(
        ...
        return {
            ...
            "by_race": {
                race: {
                    ...
                    "losses": s["losses"],
                    "win_rate": round(
                        s["wins"] / max(s["wins"] + s["losses"], 1) * 100, 2
                    ),
                }
            ...
            self._timeline.append(
                {
                    ...
                    "minerals": cost_m,
                    "vespene": cost_v,
                    "item": event.payload.get("unit_type")
                    or event.payload.get("structure_type")
                    or event.payload.get("upgrade"),
                }
            )
    ...
    def query(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "mineral_efficiency": round(
                self._total_minerals_spent
                / max(self._total_minerals_collected, 1)
                * 100,
                2,
            ),
    ...
    def apply(self, event: DomainEvent) -> None:
        if event.event_type == EventType.ATTACK_ISSUED:
            ...
            if eng_id is not None and 0 <= eng_id < len(self._engagements):
                self._engagements[eng_id]["outcome"] = event.payload.get(
                    "outcome", "unknown"
                )
...
class ReadModel:
    ...
    def __init__(self) -> None:
        ...
        self._all_projections: List[ReadModelProjection] = list(
    ...
    def execute_query(self, query: Query) -> QueryResult:
        ...
        cache_key = (
            f"{query.query_type.name}:{json.dumps(query.filters, sort_keys=True)}"
        )
        if cache_key in self._cache:
            ...
            if time.time() - cached_time < self._cache_ttl:
                ...
                return QueryResult(
                    data=cached_data, cached=True, query_time_ms=round(elapsed, 3)
                )
    ...
    def handle(self, command: Command) -> CommandResult:
        for mw in self._middleware:
            ...
            if error is not None:
                return CommandResult(
                    success=False, error=error, correlation_id=command.correlation_id
                )
...
def demo() -> None:
    ...
    # Win rate vs Protoss
    qr = bus.dispatch_query(
        Query(query_type=QueryType.WIN_RATE, filters={"opponent_race": "Protoss"})
    )
    print(f"\n[Q] Win Rate vs Protoss: {json.dumps(qr.data, indent=2)}")

    # Army composition
    qr = bus.dispatch_query(
        Query(query_type=QueryType.ARMY_COMPOSITION, filters={"include_history": True})
    )
