...
def create_game_state_table(n_rows: int = 1000) -> pa.Table:
    ...
    arrays = {
        ...
        "supply_used":   pa.array(rng.integers(12, 200, n_rows).astype(np.int8), type=pa.int8()),
        "supply_cap":    pa.array(rng.integers(14, 200, n_rows).astype(np.int8), type=pa.int8()),
        "worker_count":  pa.array(rng.integers(12, 66, n_rows).astype(np.int8), type=pa.int8()),
        "army_supply":   pa.array(rng.integers(0, 100, n_rows).astype(np.int8), type=pa.int8()),
...
def save_to_parquet(table: pa.Table, path: str, compression: str = "snappy"):
    ...
    print(f"[Arrow] Saved {len(table)} rows to {path} ({size_kb:.1f} KB, {compression})")
...
def main():
    ...
    print(f"[Arrow] Table: {game_states.num_rows} rows, {game_states.num_columns} columns")
    ...
    loaded = load_from_parquet("data/arrow/game_states.parquet",
                                columns=["game_id", "minerals", "vespene"])
