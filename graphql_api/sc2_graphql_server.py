...
class QueryParser:
    ...
    @staticmethod
    def parse(query_str: str, variables: Optional[Dict[str, Any]] = None) -> ParsedOperation:
...
class ExecutionEngine:
    ...
    def execute(
        ...
        if operation.operation_type == "query":
            root_type_name = self._schema.query_type.name if self._schema.query_type else "Query"
        elif operation.operation_type == "mutation":
            root_type_name = self._schema.mutation_type.name if self._schema.mutation_type else "Mutation"
        else:
...
class SC2Schema:
    ...
    def build(store: SC2DataStore) -> Tuple[Schema, Resolver]:
        ...
        # PlayerStats type
        stats_type = ObjectType(name="PlayerStats", description="Aggregated player statistics")
        ...
        mutation_root.add_field(Field(
            name="configureBot",
            type_name="Boolean",
            args={"aggression": "Float", "expansion_priority": "Float", "tech_priority": "Float"},
        ))
        mutation_root.add_field(Field(
            name="startTraining",
            type_name="Boolean",
        ...
        mutation_root.add_field(Field(
            name="recordGame",
            type_name="Game",
            args={"map_name": "String!", "result": "GameResult!", "duration": "Float", "strategy": "String"},
        ))
        ...
        # Query resolvers
        resolver.register("Query", "gameState", lambda p, a, c: store.games[-1] if store.games else None)

        def resolve_games(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> List[Dict]:
        ...
        def resolve_game(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> Optional[Dict]:
            gid = args.get("id", "")
        ...
        def resolve_units(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> List[Dict]:
        ...
        def resolve_buildings(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> List[Dict]:
        ...
        def resolve_player(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> Optional[Dict]:
            pid = args.get("id", "")
            return store.players.get(pid)

        resolver.register("Query", "player", resolve_player)
        resolver.register("Query", "players", lambda p, a, c: list(store.players.values()))

        def resolve_player_stats(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> Dict[str, Any]:
            pid = args.get("playerId", "")
            player = store.players.get(pid, {})
            player_games = [g for g in store.games if any(
                pl.get("id") == pid for pl in g.get("players", [])
            )]
            ...
            durations = [g.get("duration", 0) for g in player_games if g.get("duration")]
            strategies = [g.get("strategy", "unknown") for g in player_games]
            strategy_counts: Dict[str, int] = defaultdict(int)
            for s in strategies:
                strategy_counts[s] += 1
            most_played = max(strategy_counts, key=strategy_counts.get, default="none") if strategy_counts else "none"
        ...
        def resolve_resources(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> Dict[str, Any]:
            pid = args.get("playerId", "")
            return store.resources.get(pid, {"minerals": 0, "vespene": 0, "supply_used": 0, "supply_cap": 0})

        resolver.register("Query", "resources", resolve_resources)

        # Mutation resolvers
        def resolve_set_strategy(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> str:
        ...
        def resolve_configure_bot(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> bool:
            logger.info("Bot configured: aggression=%.2f expansion=%.2f tech=%.2f",
        ...
        def resolve_start_training(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> bool:
        ...
        def resolve_stop_training(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> bool:
        ...
        def resolve_record_game(parent: Any, args: Dict[str, Any], ctx: ResolverContext) -> Dict[str, Any]:
            game: Dict[str, Any] = {
...
def demo() -> None:
    ...
    for i in range(5):
        store.add_game({
            "map_name": ["Equilibrium", "GoldenAura", "HardLead", "Oceanborn", "SiteDelta"][i],
    ...
    for name, utype in [("Zergling", "light"), ("Roach", "armored"), ("Hydralisk", "ranged"),
                         ("Baneling", "light"), ("Mutalisk", "flyer")]:
        for j in range(3):
            store.add_unit({
                "name": f"{name}_{j}",
                "type": utype,
    ...
    for bname, btype in [("Hatchery", "base"), ("SpawningPool", "tech"),
                          ("Extractor", "resource"), ("RoachWarren", "tech")]:
        store.add_building({
            "name": bname,
            "type": btype,
            "health": 1500,
    ...
    print("\n--- Query: Game State ---")
    result = server.execute("query { gameState { id, map_name, result, duration, strategy } }")
    print(json.dumps(result, indent=2))

    print("\n--- Query: Match History ---")
    result = server.execute('query { games(limit: 3) { id, map_name, result, strategy } }')
    print(json.dumps(result, indent=2))

    print("\n--- Query: Units (filtered) ---")
    result = server.execute('query { units(owner: "CommanderBot", limit: 5) { id, name, type, health } }')
    print(json.dumps(result, indent=2))

    print("\n--- Query: Buildings ---")
    result = server.execute('query { buildings(type: "tech") { id, name, is_completed } }')
    print(json.dumps(result, indent=2))

    print("\n--- Query: Player ---")
    result = server.execute(f'query {{ player(id: "{pid1}") {{ id, name, race, mmr, win_rate }} }}')
    print(json.dumps(result, indent=2))

    print("\n--- Query: Player Stats ---")
    result = server.execute(f'query {{ playerStats(playerId: "{pid1}") {{ total_games, wins, losses, win_rate, avg_game_duration, most_played_strategy }} }}')
    print(json.dumps(result, indent=2))

    print("\n--- Query: Resources ---")
    result = server.execute(f'query {{ resources(playerId: "{pid1}") {{ minerals, vespene }} }}')
    print(json.dumps(result, indent=2))
    ...
    result = server.execute('mutation { configureBot(aggression: 0.8, expansion_priority: 0.3, tech_priority: 0.6) }')
    print(json.dumps(result, indent=2))

    print("\n--- Mutation: Start Training ---")
    result = server.execute('mutation { startTraining(episodes: 500, opponent: "very-hard-ai") }')
    print(json.dumps(result, indent=2))

    print("\n--- Mutation: Record Game ---")
    result = server.execute('mutation { recordGame(map_name: "Equilibrium", result: "WIN", duration: 420.0, strategy: "roach-timing") }')
    print(json.dumps(result, indent=2))
