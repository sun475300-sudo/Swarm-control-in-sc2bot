...
export async function getCurrentGameSession(): Promise<GameSession | null> {
  try {
    const response = await api.post<ApiResponse<GameSession | null>>(
...
export async function getGameSessions(
  ...
  try {
    const response = await api.post<ApiResponse<GameSession[]>>(
      "game.getSessions",
      {
        json: { limit },
      }
...
// 학습 에피소드 목록 조회
export async function getTrainingEpisodes(
  limit: number = 50
): Promise<TrainingEpisode[]> {
  try {
    const response = await api.post<ApiResponse<TrainingEpisode[]>>(
...
export async function getTrainingStats(): Promise<TrainingStats | null> {
  try {
    const response = await api.post<ApiResponse<TrainingStats>>(
      "training.getStats",
      {
        json: {},
      }
...
export async function getBotConfigs(): Promise<BotConfig[]> {
  try {
    const response = await api.post<ApiResponse<BotConfig[]>>(
      "bot.getConfigs",
      {
        json: {},
      }
...
export async function getArenaMatches(
  ...
  try {
    const response = await api.post<ApiResponse<ArenaMatch[]>>(
      "arena.getMatches",
      {
        json: { limit },
      }
