...
SQL_INJECTION_PATTERNS: list[tuple[str, str]] = [
    (r'execute\s*\(\s*[f"\'](SELECT|INSERT|UPDATE|DELETE|DROP)', "Raw SQL in execute()"),
...
def check_assert_usage(content: str, filepath: str) -> list[Finding]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assert):
            ...
            if any(kw in src.lower() for kw in ["auth", "permission", "role", "token", "admin"]):
                findings.append(Finding(
                    severity="HIGH",
...
def audit(root: str = ".") -> list[Finding]:
    ...
    for filepath in py_files:
        ...
        all_findings += scan_patterns(content, rel, SECRET_PATTERNS, "CRITICAL", "hardcoded_secrets")
        all_findings += scan_patterns(content, rel, SQL_INJECTION_PATTERNS, "HIGH", "sql_injection")
        all_findings += scan_patterns(content, rel, CMD_INJECTION_PATTERNS, "HIGH", "command_injection")
        all_findings += scan_patterns(content, rel, DESERIALIZATION_PATTERNS, "HIGH", "insecure_deserialization")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="security_report.json", help="Output JSON report path")
