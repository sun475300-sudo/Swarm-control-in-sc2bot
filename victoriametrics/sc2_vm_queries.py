...
def push_metric(
    metric_name: str, value: float, labels: dict, timestamp: Optional[int] = None
):
    ...
    resp = requests.post(
        f"{VMINSERT_URL}/api/v1/import/prometheus",
        data=line,
...
def query_instant(metricsql: str) -> dict:
    ...
    resp = requests.get(f"{VMSELECT_URL}/api/v1/query", params={"query": metricsql})
    resp.raise_for_status()
    return resp.json()


def query_range(
    metricsql: str, start: datetime, end: datetime, step: str = "1m"
) -> dict:
    """Range query against vmselect."""
    resp = requests.get(
        f"{VMSELECT_URL}/api/v1/query_range",
        params={
            "query": metricsql,
