...
- **2026-07-03** — Resumed daily test/improve/commit loop (branch `claude/optimistic-edison-96ullp`). Fixed: (1) `tests/test_queen_transfusion.py` collection error — bare `from sc2...` import crashed collection when `sc2` package absent, now guarded with `pytest.skip`; (2) `tests/test_combat_phase_fsm.py` — 12 tests used `asyncio.get_event_loop().run_until_complete(...)`, which raises `RuntimeError: no current event loop` under recent pytest-asyncio — replaced with `asyncio.run(...)`; (3) `.github/workflows/sc2bot-ci.yml` — "Run unit tests" step pointed at `tests/unit`, a directory that doesn't exist, so the job would fail to collect anything — repointed to `pytest tests/ --ignore=tests/integration`. **Result: 395 pass / 34 skip / 0 fail** (up from 83 failing in a clean env missing `pytest-asyncio`/`cffi`). Re-verified `REMAINING_ISSUES.md` N1-N4 (F811 duplicate defs) — confirmed already resolved by commit `e648ae4`, marked resolved in doc. Re-verified Issue #5 (position-calc duplication) — `utils/position_utils.py` exists but is unused by any caller; duplication still present in 18 files/20+ call sites — flagged as top backlog item for a dedicated, carefully-tested follow-up cycle rather than a blind bulk replace.
