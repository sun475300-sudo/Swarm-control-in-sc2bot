...
| P2.6 | CI lint job fails on repo-wide `black`/`isort`  | ❌ Open (found 2026-07-08) | `sc2bot-ci.yml` lint job runs `black --check --diff .` / `isort --check-only --diff .` over the whole repo, blocking, `needs: lint` gates the test job. At least `wicked_zerg_challenger/local_training/rl_agent.py` fails pre-existing (unrelated to this session's edits). Two options: (a) scope the check to `wicked_zerg_challenger/`, `tests/`, `src/` instead of `.`; (b) run `black .`/`isort .` repo-wide once and commit — high blast radius across 280+ scaffold dirs, needs explicit sign-off before attempting. |
