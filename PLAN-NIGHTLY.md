...
| P2.7 | Repo-wide `black` formatting debt                | ❌ Open (new) | `.github/workflows/ci.yml`'s "Lint & Type Check" job runs `black --check --diff .` over the *whole* repo and already fails on `main` HEAD (`8a80b73`, confirmed via Actions history) — 66 files need reformatting (mostly `wicked_zerg_challenger/tests/*`, `strategy_manager.py`, `visuals/generate_*.py`). Pre-existing, unrelated to PR #384's diff (none of PR #384's touched files are in the 66). Needs its own dedicated PR (`black .` + verify tests still pass) rather than folding into unrelated work. |
