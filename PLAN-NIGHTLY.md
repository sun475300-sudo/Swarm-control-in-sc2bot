...
| **CI "Python 린트 & 테스트" job broken repo-wide** (`TypeError: Descriptors cannot be created directly`) | `requirements.txt` | Found while triaging PR #380's failing checks. `requirements.txt` pinned a floor-only `s2clientprotocol>=4.19.0.0` *in addition to* `burnysc2>=5.0.0`. `burnysc2` already depends on `pys2clientprotocol`, which installs protobuf-runtime-compatible generated files under the same `s2clientprotocol` import path — but pip's install order lets the standalone `s2clientprotocol` package overwrite them with old protoc-generated code, which crashes on import under any current `protobuf` release. Reproduced in an isolated venv: `pip install burnysc2` alone imports fine; adding `s2clientprotocol>=4.19.0.0` on top breaks `from sc2.position import Point2` immediately. Confirmed via `git worktree` that this reproduces identically on `main` (pre-existing, unrelated to this PR's diff) — removed the redundant line and documented why in a comment so it doesn't get re-added. |
