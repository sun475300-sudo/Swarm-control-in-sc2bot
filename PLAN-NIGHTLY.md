...
- **2026-07-03 (session `optimistic-edison-9ze9sz`)** — **Process finding: 30 open draft PRs (`claude/optimistic-edison-*`, #227–#256) already fix the same `tests/test_combat_phase_fsm.py` `asyncio.get_event_loop()` crash and none are merged.** Applied that fix here too (needed for a clean local baseline) plus a genuinely new bug not covered by any of those PRs: `pytest tests/ wicked_zerg_challenger/tests/` together threw 2 collection errors (`scripts.ladder_tracker`, `scripts.meta_adapter` not found) because `tests/test_production_resilience.py` put `wicked_zerg_challenger/local_training` directly on `sys.path`, which made `wicked_zerg_challenger/local_training/scripts/` (a real package) permanently shadow the repo-root `scripts/` namespace package for the rest of the pytest session — order-independent, since regular packages always beat namespace packages regardless of `sys.path` position. Removed the redundant path insert; converted a few sibling `sys.path.insert(0, ...)` calls that only needed to add a subpackage (not shadow the root) to `sys.path.append(...)`. Also closed the last "open" REMAINING_ISSUES.md items (#3 transfusion priority, #4 resource-reservation lock) — both were already implemented in code, doc was stale. Result: `pytest tests/` 505 pass/11 skip/0 fail, `pytest wicked_zerg_challenger/tests/` 661 pass/0 fail, combined run also clean; flake8 `E9,F63,F7,F82` gate clean; black/isort clean on touched files.
  **Recommendation for the repo owner:** merge PR #256 (most recent/comprehensive of the 30 duplicates) and close the other 29 — every future nightly session re-discovers and re-fixes the same bug because main never advances, which is pure wasted work until one of these actually merges.
