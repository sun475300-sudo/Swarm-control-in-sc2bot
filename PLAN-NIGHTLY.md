...
- Branch: `main` (via PR #239 on `claude/optimistic-edison-xmhc91`).
- Bot core: `wicked_zerg_challenger/` — 179+ Python files across 10+ subdirs.
- `.gitattributes` enforces `* text=auto` ✅
- CI: had been failing on every push/schedule since 2026-05-28 (black/isort
  drift blocked the lint gate, hiding a `tests/unit`-path bug in the Test
  Suite job and a protobuf/pytest-timeout gap in two other jobs). All fixed
  2026-07-03 — full pipeline (23 checks incl. Test Suite, SC2 bot validation,
  CodeQL, Docker build) is green again.
- **Test suite: 476 pass / 12 skip / 0 fail** ✅ (12 failures from a
  pytest-asyncio incompatibility fixed same day)
...
| P2.4 | RL agent save-experience guard                  | ✅ Done | Fixed remove+rename data-loss window with `os.replace()`; 7 tests in `wicked_zerg_challenger/tests/test_rl_agent_save_guard.py`. |
...
- **2026-07-03** — **CI unblocked after ~5 weeks red:** black/isort drift (70 files) fixed; `sc2bot-ci.yml` Test Suite job's `tests/unit` path (never existed) corrected to `tests/`; `PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python` added to two jobs missing it (protobuf 5.x vs. s2clientprotocol's generated code); `pytest-timeout` added to Test Suite's install step; 12 `test_combat_phase_fsm.py` failures from deprecated `asyncio.get_event_loop().run_until_complete()` fixed via `asyncio.run()`. P2.4 (RL save-experience guard) closed: `os.replace()` fix + 7 new tests. Full CI green (23/23 checks) for the first time since 2026-05-28. See PR #239.
