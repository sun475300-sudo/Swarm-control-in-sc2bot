...
| P0.3 | CI `ci.yml` "Python 린트 & 테스트" fails on fresh `pip install -r requirements.txt` | ❌ Open, pre-existing | `TypeError: Descriptors cannot be created directly` — the official `s2clientprotocol` package (pinned `>=4.19.0.0` in `requirements.txt`) ships `_pb2.py` files that are sometimes incompatible with the resolved `protobuf` runtime; `burnysc2` already depends on the community-maintained `pys2clientprotocol` (same `s2clientprotocol` import path, protobuf-compatible), so the two collide and whichever installs last wins nondeterministically. Confirmed **pre-existing**: `main`'s current HEAD (`8a80b73`, before this PR branched) already has this workflow failing (`gh` run history: failure on `8a80b73`, `1b1a9bf4`, `3ace0181`, etc., mixed with occasional `success`). Reproducing needs care — a minimal `burnysc2 + s2clientprotocol>=4.19.0.0` install in isolation did *not* reproduce it, so it's sensitive to the full 60+ line `requirements.txt` resolving other transitive constraints. Candidate fix: drop the redundant explicit `s2clientprotocol>=4.19.0.0` pin from `requirements.txt` (rely on `burnysc2`'s own `pys2clientprotocol` dependency) — not applied yet since it touches a shared file across every subsystem (crypto trading, Discord bot, GenAI, AWS) and deserves its own PR + verification, not a drive-by fix while investigating an unrelated CI ping. |
