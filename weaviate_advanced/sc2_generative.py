...
from weaviate.classes.query import (
    MetadataQuery, HybridFusion, Generate, NearText,
...
def hybrid_search(query: str, matchup: Optional[str] = None, alpha: float = 0.5, limit: int = 10):
...
if __name__ == "__main__":
    create_schema()
    strategies = [
        {"strategy_name": "Ling Bane Muta", "race": "Zerg", "matchup": "ZvT",
         "description": "Economic macro build transitioning into Ling Bane Muta",
         "build_order": "Pool → Gas → Hatch → Lair → Bane Nest → Spire",
         "win_rate": 0.58, "is_meta": True, "tags": ["macro", "ling", "bane", "muta"]},
    ]
    insert_strategies(strategies)

    results = near_text_search(["aggressive early pressure", "zergling rush"], race="Zerg")
