...
def recommend_action(
    ...
    recommendations = {
        ("early", "Terran"):  "Scout with overlord, build spine crawler at natural, prepare zergling speed",
        ("early", "Protoss"): "Scout for 4-gate or stargate, build roach warren defensively",
        ("early", "Zerg"):    "Check for pool timing, match unit production, prepare speedlings",
        ("mid", "Terran"):    "Go roach-hydra with +1/+1 attack, deny third base, attack at 9 min",
        ("mid", "Protoss"):   "Roach-ravager-hydra with corruptors for colossus, fungal forcefield",
        ("mid", "Zerg"):      "Muta-ling play or roach-hydra, control map with overlord vision",
        ("late", "Terran"):   "Brood lord + infestor + corruptor vs mech, ultra-ling vs bio",
...
def build_order_advisor(
    ...
    build_orders = {
        ("Terran", "aggressive"): [
            "12 Pool", "14 Hatchery", "16 Gas", "18 Zergling speed",
            "20 Hatchery", "4:00 Roach Warren", "6:00 attack with 12 roaches+lings"
        ],
        ("Terran", "economic"): [
            "17 Hatchery", "17 Gas", "16 Pool", "21 Hatchery",
            "Queen x3", "Roach Warren at 5:00", "Macro up to 70 drones"
        ],
        ("Protoss", "defensive"): [
            "17 Hatchery", "16 Pool", "17 Gas x2", "Roach Warren at 4:30",
            "Lair at 5:30", "Hydra Den at 6:30", "Ravager research"
        ],
    }
    key = (enemy_race, style)
    steps = build_orders.get(key, ["17 Hatch", "16 Pool", "Standard macro opener"])
    return json.dumps({"build_order": steps, "matchup": f"Zv{enemy_race[0]}", "style": style})
...
def create_sc2_group_chat(game_state: dict) -> str:
    ...
    for agent in [strategist, economy, scout, combat]:
        autogen.register_function(
            analyze_game_state,
            caller=agent, executor=user_proxy,
