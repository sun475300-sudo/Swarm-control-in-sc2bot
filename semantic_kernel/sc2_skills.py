...
class StrategyPlugin:
    ...
    @kernel_function(name="analyze_matchup", description="Analyze a SC2 matchup and return strategy")
    def analyze_matchup(
        self,
        ...
        strategies = {
            ...
            ("Zerg", "Protoss", "mid"): "Roach-Ravager-Hydra with Corruptors for Colossus",
    ...
    @kernel_function(name="get_counter_unit", description="Get the counter unit for an enemy composition")
...
class EconomyPlugin:
    ...
    @kernel_function(name="calculate_drone_target", description="Calculate optimal drone count")
    ...
    @kernel_function(name="check_supply_block", description="Check if supply is blocked")
    def check_supply_block(
        ...
        return json.dumps({
            ...
            "overlords_needed": max(0, (supply_used - supply_cap + 10) // 8 + 1) if is_blocked else 0,
        })

class CombatPlugin:
    """SC2 combat management plugin."""

    @kernel_function(name="evaluate_attack_timing", description="Evaluate whether to attack now")
        ...
        for start, end, min_supply, label in timing_windows:
            if start <= game_minute <= end and army_supply >= min_supply:
                ...
                return json.dumps({
                    ...
                    "window": label,
                    "reason": "Timing window + supply advantage" if should_attack else "Timing window but supply deficit",
                })
    ...
    @kernel_function(name="get_army_composition", description="Get recommended army composition")
...
async def run_strategic_planner(kernel: sk.Kernel, goal: str) -> str:
    ...
    planner = FunctionCallingStepwisePlanner(service_id="anthropic-claude", options=options)
...
async def main():
    ...
    attack_result  = await kernel.invoke(
        ...
        army_supply=42, enemy_army_supply=30, game_minute=9.5
    )
