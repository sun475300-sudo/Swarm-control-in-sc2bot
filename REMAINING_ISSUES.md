...
**Last refreshed:** 2026-07-10 (N1-N4 verified resolved by PR #218; see below)
...
| ID | 설명 | 우선순위 | 상태 |
|----|------|---------|------|
| N1 | `OpponentModeling.on_step` 중복 정의 (line 341 vs 765 — F811) | 🟠 HIGH | ✅ resolved (PR #218) — 중복 정의 제거 확인 (`flake8 --select=F811` 0건) |
| N2 | `EconomyManager._prevent_resource_banking` / `_reduce_gas_workers` 재정의 (F811) | 🟡 MED | ✅ resolved (PR #218) — 각각 단일 정의만 존재 |
| N3 | `combat_manager._find_harass_target` 재정의 (line 2377 vs 4278) | 🟡 MED | ✅ resolved (PR #218) — 단일 정의만 존재 |
| N4 | `production_resilience.build_terran_counters` 재정의 (1369 vs 1866) | 🟡 MED | ✅ resolved (PR #218) — 단일 정의만 존재 |
| N5 | bare `except Exception:` 다수 (≈360+) — 이번 PR에서 12건 처리, 잔여 다수 | 🟢 LOW | partial (2026-07-10 기준 `wicked_zerg_challenger/` 내 460건 잔존, 대부분 SC2 API 방어 코드) |
| N6 | F841 unused local variables (visuals/make_pptx 등) | 🟢 LOW | open (2026-07-10 기준 4건 잔존, presentation 코드라 영향 작음) |

N1-N4는 2026-07-10 점검 사이클에서 `flake8 --select=F811 wicked_zerg_challenger/`로 재검증 완료 (0건). N5/N6은 여전히 낮은 우선순위로 open 상태 유지.
