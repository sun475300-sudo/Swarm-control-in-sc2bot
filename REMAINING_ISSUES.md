...
**Last refreshed:** 2026-07-16 (verified directly against source — N1-N4, Issue #3, #4, #5 confirmed resolved)

---

## 🆕 2026-04-27 항목 재검증 결과 (2026-07-16)

이 문서에 "open"으로 남아있던 N1-N4는 실제 코드를 `grep`으로 직접 확인한 결과 **중복 정의가 존재하지 않음** —
이미 다른 세션에서 고쳐졌으나 이 문서가 갱신되지 않았던 것(stale docs).

| ID | 설명 | 재검증 결과 |
|----|------|------|
| N1 | `OpponentModeling.on_step` 중복 정의 | ✅ 해결됨 — `on_step` 단일 정의만 존재 (opponent_modeling.py:341) |
| N2 | `EconomyManager._prevent_resource_banking`/`_reduce_gas_workers` 재정의 | ✅ 해결됨 — 각각 단일 정의만 존재 |
| N3 | `combat_manager._find_harass_target` 재정의 | ✅ 해결됨 — 단일 정의(4992), 나머지는 호출부 |
| N4 | `production_resilience.build_terran_counters` 재정의 | ✅ 해결됨 — 단일 정의만 존재 |
| N5 | bare `except Exception:` 다수 | 🟢 LOW — 잔존 (점진적 개선 대상, 안정성에 즉각적 영향 없음) |
| N6 | F841 unused locals (visuals/) | 🟢 LOW — 잔존 (프레젠테이션 코드, 영향 작음) |
...
## ✅ Resolved (재검증: 2026-07-16)

### ✅ Issue #3: Transfusion 우선순위 — 구현 완료

`wicked_zerg_challenger/economy/queen_transfusion_manager.py`의 `QueenTransfusionManager`가
`HEAL_PRIORITY` 딕셔너리 기반 우선순위 시스템을 이미 구현하고 있음 (line 26+).
아래 원안과 동일한 목적의 로직이 존재하며 회귀 테스트도 `tests/test_queen_transfusion.py`에 있음.

### ✅ Issue #4: Resource Reservation Race Condition — 구현 완료

`wicked_zerg_challenger/core/resource_manager.py`의 `ResourceManager`가
`asyncio.Lock()` 기반 `try_reserve`/`release`를 이미 구현하고 있음 (line 28-127).
회귀 테스트는 `tests/test_resource_manager.py`에 존재.

### ✅ Issue #5: 코드 중복 - Position 계산 — 이번 세션에서 완료

`utils/position_utils.py`에 `get_center_position` 등 헬퍼가 존재했지만 실제로는
**아무 파일에서도 import되지 않고 있었음** (이전 PR #477이 이 마이그레이션을 시도했지만 병합되지 않아 유실됨).
이번 세션에서 `battle_preparation_system.py::_find_enemy_clusters`의 인라인 centroid 계산을
`get_center_position()` 호출로 교체하고 `tests/test_battle_preparation_system.py`(3 tests)를 추가함.

`combat_manager.py`의 나머지 2개 사이트(`_get_units_center`, `_get_enemy_center`)는
sc2 미설치 환경을 위한 방어적 fallback 로직이 얽혀 있어 순수 스타일 정리 목적으로
건드리지 않음 — 실제 버그가 아니므로 행동 변경 리스크를 감수할 이유가 없음.

---

## 🟡 MEDIUM Priority Issues (historical — superseded by "Resolved" section above)
...
| 우선순위 | 이슈 | 상태 |
|---------|------|------|
| 🟢 LOW | N5 bare `except Exception:` 잔존 정리 | open — 점진적 개선 |
| 🟢 LOW | N6 F841 unused locals (visuals/) | open — 영향 작음 |
| 🟢 LOW | #6 매직 넘버 | open — 가독성 개선, 급하지 않음 |

Issue #1-#5 → 모두 ✅ Resolved 섹션 참조 (코드에서 직접 검증 완료).
...
- N1-N4 F811 중복 정의 (실제로 중복 없음 확인)
- Transfusion 우선순위 시스템 (`queen_transfusion_manager.py`)
- Resource Reservation 동기화 (`core/resource_manager.py`)
- Position Utils 유틸리티 적용 (`battle_preparation_system.py`)

### 남은 작업 (LOW priority, 미진행)
- bare `except Exception:` 잔존분 로깅 추가
- F841 unused locals 정리 (visuals/)
- Constants 정리 (매직 넘버 → named constants)
