...
| N11 | `.github/workflows/ci.yml`(JARVIS CI/CD)의 "Python 린트 & 테스트" job이 `pip install -r requirements.txt`로 SC2 봇 + 크립토 트레이딩 + Discord봇 + Google API + AWS 등 무관한 서비스 의존성을 전부 설치하는데, 이 과정에서 설치되는 `protobuf`가 `s2clientprotocol`이 생성한 `_pb2.py`와 버전이 맞지 않아 `TypeError: Descriptors cannot be created directly`로 `tests/` 전체 collect가 깨짐(sc2 관련 테스트 14개 collection error). main 최신 커밋(8a80b7357a, 2026-06-25)에서도 동일하게 실패 중인 사전 존재 문제로 확인됨 — 이번 PR과 무관 | 🟡 MED | ⚠️ OPEN, 범위 밖 — 원인은 파악됐으나 하나의 거대한 `requirements.txt`에 여러 무관한 서비스가 뒤섞여 있어 protobuf 핀을 바꾸면 크립토/Discord/구글 API 등 이 세션에서 검증 불가능한 다른 서비스가 깨질 위험이 있음. 별도 PR로 `requirements.txt`를 서비스별로 분리(`requirements-sc2bot.txt`, `requirements-crypto.txt` 등은 이미 일부 존재)하거나, `ci.yml`의 python-lint-test job이 SC2 관련 테스트를 collect하지 않도록 범위를 좁히는 방향을 권장 |
