...
class AutoTrader:
    ...
    def __init__(self):
        ...
        self.buy_threshold: int = 30           # 이 점수 이상이면 자동 매수
        self.sell_threshold: int = -30         # 이 점수 이하이면 자동 매도
    ...
    def _run_smart_cycle(self) -> dict:
        ...
        try:
            ...
            for b in balances:
                ...
                if action == "stop_loss":
                    order = self.client.sell_market_order(ticker, balance)
                    self.tracker.log_trade("sell", ticker, balance * current_price,
                                           current_price, "손절", order)
                    result["actions"].append(f"손절 매도: {ticker} x {balance}")
                elif action == "take_profit":
                    order = self.client.sell_market_order(ticker, balance)
                    self.tracker.log_trade("sell", ticker, balance * current_price,
                                           current_price, "익절", order)
                    result["actions"].append(f"익절 매도: {ticker} x {balance}")
        ...
        result["positions"] = current_positions if 'current_positions' in dir() else 0
